...
def transaction_payment_method(data: dict) -> 'TransactionPaymentMethod':
    ...
    if not data:
        return None
    return TransactionPaymentMethod(id=TransactionPaymentMethodIds.__members__.get(data.get('id')), name=data.get('name'), fee=data.get('fee'), provider_id=TransactionProviderIds.__members__.get(data.get('provider_id')), account=account_profile(data.get('account')), props=transaction_provider_props(data.get('props')), limits=transaction_provider_limits(data.get('limits')))
...
def transaction_provider_limits(data: dict) -> 'TransactionProviderLimits':
    ...
    if not data:
        return None
    return TransactionProviderLimits(incoming=transaction_provider_limit_range(data.get('incoming')), outgoing=transaction_provider_limit_range(data.get('outgoing')))
...
def transaction_provider_props(data: dict) -> 'TransactionProviderProps':
    ...
    if not data:
        return None
    return TransactionProviderProps(required_user_data=transaction_provider_required_user_data(data.get('requiredUserData')), tooltip=data.get('tooltip'))
...
def transaction_provider(data: dict) -> 'TransactionProvider':
    ...
    if not data:
        return None
    return TransactionProvider(id=TransactionProviderIds.__members__.get(data.get('id')), name=data.get('name'), fee=data.get('fee'), min_fee_amount=data.get('minFeeAmount'), description=data.get('description'), account=account_profile(data.get('account')), props=transaction_provider_props(data.get('props')), limits=transaction_provider_limits(data.get('limits')), payment_methods=[transaction_payment_method(method) for method in data.get('paymentMethods') or []])

def transaction(data: dict) -> 'Transaction':
    from .types import Transaction
    if not data:
        return None
    return Transaction(id=data.get('id'), operation=TransactionOperations.__members__.get(data.get('operation')), direction=TransactionDirections.__members__.get(data.get('direction')), provider_id=TransactionProviderIds.__members__.get(data.get('providerId')), provider=transaction_provider(data.get('provider')), user=user_profile(data.get('user')), creator=user_profile(data.get('creator')), status=TransactionStatuses.__members__.get(data.get('status')), status_description=data.get('statusDescription'), status_expiration_date=data.get('statusExpirationDate'), value=data.get('value'), fee=data.get('fee'), created_at=data.get('createdAt'), verified_at=data.get('verified_at'), verified_by=data.get('verified_by'), completed_at=data.get('completed_at'), completed_by=data.get('completed_by'), payment_method_id=data.get('paymentMethodId'), is_suspicious=data.get('is_suspicious'), sbp_bank_name=data.get('spb_bank_name'))
...
def transaction_list(data: dict) -> 'TransactionList':
    ...
    if not data:
        return None
    return TransactionList(transactions=[transaction(edge.get('node')) for edge in data.get('edges') or []], page_info=transaction_page_info(data.get('pageInfo')), total_count=data.get('totalCount'))
...
def user_bank_card_list(data: dict) -> 'UserBankCardList':
    ...
    if not data:
        return None
    return UserBankCardList(bank_cards=[user_bank_card(edge.get('node')) for edge in data.get('edges') or []], page_info=user_bank_card_page_info(data.get('pageInfo')), total_count=data.get('totalCount'))

def game_category_data_field(data: dict) -> 'GameCategoryDataField':
    from .types import GameCategoryDataField
    if not data:
        return None
    return GameCategoryDataField(id=data.get('id'), label=data.get('label'), type=GameCategoryDataFieldTypes.__members__.get(data.get('type')), input_type=GameCategoryDataFieldInputTypes.__members__.get(data.get('inputType')), copyable=data.get('copyable'), hidden=data.get('hidden'), required=data.get('required'), value=data.get('value'))
...
def game_category_data_field_list(data: dict) -> 'GameCategoryDataFieldList':
    ...
    if not data:
        return None
    return GameCategoryDataFieldList(data_fields=[game_category_data_field(edge.get('node')) for edge in data.get('edges') or []], page_info=game_category_data_field_page_info(data.get('pageInfo')), total_count=data.get('totalCount'))
...
def game_category_option(data: dict) -> 'GameCategoryOption':
    ...
    if not data:
        return None
    return GameCategoryOption(id=data.get('id'), group=data.get('group'), label=data.get('label'), type=GameCategoryOptionTypes.__members__.get(data.get('type')), field=data.get('field'), value=data.get('value'), value_range_limit=data.get('valueRangeLimit'))
...
def game_category_agreement_list(data: dict) -> 'GameCategoryAgreementList':
    ...
    if not data:
        return None
    return GameCategoryAgreementList(agreements=[game_category_agreement(edge.get('node')) for edge in data.get('edges') or []], page_info=game_category_agreement_page_info(data.get('pageInfo')), total_count=data.get('totalCount'))

def game_category_obtaining_type(data: dict) -> 'GameCategoryObtainingType':
    from .types import GameCategoryObtainingType
    if not data:
        return None
    return GameCategoryObtainingType(id=data.get('id'), name=data.get('name'), description=data.get('description'), game_category_id=data.get('gameCategoryId'), no_comment_from_buyer=data.get('noCommentFromBuyer'), instruction_for_buyer=data.get('instructionForBuyer'), instruction_for_seller=data.get('instructionForSeller'), sequence=data.get('sequence'), fee_multiplier=data.get('feeMultiplier'), agreements=[game_category_agreement(agr) for agr in data.get('agreements') or []], props=game_category_props(data.get('props')))
...
def game_category_obtaining_type_list(data: dict) -> 'GameCategoryObtainingTypeList':
    ...
    if not data:
        return None
    return GameCategoryObtainingTypeList(obtaining_types=[game_category_obtaining_type(edge.get('node')) for edge in data.get('edges') or []], page_info=game_category_obtaining_type_page_info(data.get('pageInfo')), total_count=data.get('totalCount'))
...
def game_category_instruction_list(data: dict) -> 'GameCategoryInstructionList':
    ...
    if not data:
        return None
    return GameCategoryInstructionList(instructions=[game_category_instruction(edge.get('node')) for edge in data.get('edges') or []], page_info=game_category_instruction_page_info(data.get('pageInfo')), total_count=data.get('totalCount'))

def game_category(data: dict) -> 'GameCategory':
    from .types import GameCategory
    if not data:
        return None
    return GameCategory(id=data.get('id'), slug=data.get('slug'), name=data.get('name'), category_id=data.get('categoryId'), game_id=data.get('gameId'), obtaining=data.get('obtaining'), options=[game_category_option(option) for option in data.get('options') or []], props=game_category_props(data.get('props')), no_comment_from_buyer=data.get('noCommentFromBuyer'), instruction_for_buyer=data.get('instructionForBuyer'), instruction_for_seller=data.get('instructionForSeller'), use_custom_obtaining=data.get('useCustomObtaining'), auto_confirm_period=GameCategoryAutoConfirmPeriods.__members__.get(data.get('autoConfirmPeriod')), auto_moderation_mode=data.get('autoModerationMode'), agreements=[game_category_agreement(agr) for agr in data.get('agreements') or []], fee_multiplier=data.get('feeMultiplier'))

def game(data: dict) -> 'Game':
    from .types import Game
    if not data:
        return None
    return Game(id=data.get('id'), slug=data.get('slug'), name=data.get('name'), type=GameTypes.__members__.get(data.get('type')), logo=file(data.get('logo')), banner=file(data.get('banner')), categories=[game_category(cat) for cat in data.get('categories') or []], created_at=data.get('createdAt'))

def game_profile(data: dict) -> 'GameProfile':
    from .types import GameProfile
    if not data:
        return None
    return GameProfile(id=data.get('id'), slug=data.get('slug'), name=data.get('name'), type=GameTypes.__members__.get(data.get('type')), logo=file(data.get('logo')))
...
def account_deals_stats(data: dict) -> 'AccountDealsStats':
    ...
    if not data:
        return None
    return AccountDealsStats(incoming=account_incoming_deals_stats(data.get('incoming')), outgoing=account_outgoing_deals_stats(data.get('outgoing')))

def account_stats(data: dict) -> 'AccountStats':
    from .types import AccountStats
    if not data:
        return None
    return AccountStats(items=account_items_stats(data.get('items')), deals=account_deals_stats(data.get('deals')))
...
def item_priority_status(data: dict) -> 'ItemPriorityStatus':
    ...
    if not data:
        return None
    return ItemPriorityStatus(id=data.get('id'), price=data.get('price'), name=data.get('name'), type=PriorityTypes.__members__.get(data.get('type')), period=data.get('period'), price_range=item_priority_status_price_range(data.get('priceRange')))

def item_log(data: dict) -> 'ItemLog':
    from .types import ItemLog
    if not data:
        return None
    return ItemLog(id=data.get('id'), event=ItemLogEvents.__members__.get(data.get('event')), created_at=data.get('createdAt'), user=user_profile(data.get('user')))

def item(data: dict) -> 'Item':
    from .types import Item
    if not data:
        return None
    return Item(id=data.get('id'), slug=data.get('slug'), name=data.get('name'), description=data.get('description'), obtaining_type=game_category_obtaining_type(data.get('obtainingType')), price=data.get('price'), raw_price=data.get('rawPrice'), priority_position=data.get('priorityPosition'), attachments=[file(att) for att in data.get('attachments') or []], attributes=data.get('attributes'), category=game_category(data.get('category')), comment=data.get('comment'), data_fields=[game_category_data_field(field) for field in data.get('dataFields') or []], fee_multiplier=data.get('feeMultiplier'), game=game_profile(data.get('game')), seller_type=data.get('sellerType'), status=ItemStatuses.__members__.get(data.get('status')), user=user_profile(data.get('user')))

def my_item(data: dict) -> 'MyItem':
    from .types import MyItem
    if not data:
        return None
    return MyItem(id=data.get('id'), slug=data.get('slug'), name=data.get('name'), description=data.get('description'), obtaining_type=game_category_obtaining_type(data.get('obtainingType')), price=data.get('price'), prev_price=data.get('prevPrice'), raw_price=data.get('rawPrice'), priority_position=data.get('priorityPosition'), attachments=[file(att) for att in data.get('attachments') or []], attributes=data.get('attributes'), buyer=user_profile(data.get('buyer')), category=game_category(data.get('category')), comment=data.get('comment'), data_fields=[game_category_data_field(field) for field in data.get('dataFields') or []], fee_multiplier=data.get('feeMultiplier'), prev_fee_multiplier=data.get('prevFeeMultiplier'), seller_notified_about_fee_change=data.get('sellerNotifiedAboutFeeChange'), game=game_profile(data.get('game')), seller_type=data.get('sellerType'), status=ItemStatuses.__members__.get(data.get('status')), user=user_profile(data.get('user')), priority=PriorityTypes.__members__.get(data.get('priority')), priority_price=data.get('priorityPrice'), sequence=data.get('sequence'), status_expiration_date=data.get('statusExpirationDate'), status_description=data.get('statusDescription'), status_payment=transaction(data.get('statusPayment')), views_counter=data.get('viewsCounter'), is_editable=data.get('isEditable'), approval_date=data.get('approvalDate'), deleted_at=data.get('deletedAt'), updated_at=data.get('updatedAt'), created_at=data.get('createdAt'))

def item_profile(data: dict) -> 'ItemProfile':
    from .types import ItemProfile
    if not data:
        return None
    return ItemProfile(id=data.get('id'), slug=data.get('slug'), priority=PriorityTypes.__members__.get(data.get('priority')), status=ItemStatuses.__members__.get(data.get('status')), name=data.get('name'), price=data.get('price'), raw_price=data.get('rawPrice'), seller_type=UserTypes.__members__.get(data.get('sellerType')), attachment=file(data.get('attachment')), user=user_profile(data.get('user')), approval_date=data.get('approvalDate'), priority_position=data.get('priorityPosition'), views_counter=data.get('viewsCounter'), fee_multiplier=data.get('feeMultiplier'), created_at=data.get('createdAt'))
...
def item_profile_list(data: dict) -> 'ItemProfileList':
    ...
    if not data:
        return None
    return ItemProfileList(items=[item_profile(edge.get('node')) for edge in data.get('edges') or []], page_info=item_profile_page_info(data.get('pageInfo')), total_count=data.get('totalCount'))
...
def chat(data: dict) -> 'Chat':
    ...
    if not data:
        return None
    return Chat(id=data.get('id'), type=ChatTypes.__members__.get(data.get('type')), status=ChatStatuses.__members__.get(data.get('status')), unread_messages_counter=data.get('unreadMessagesCounter'), bookmarked=data.get('bookmarked'), is_texting_allowed=data.get('isTextingAllowed'), owner=user_profile(data.get('owner')), deals=[item_deal(deal) for deal in data.get('deals') or []], started_at=data.get('startedAt'), finished_at=data.get('finishedAt'), last_message=chat_message(data.get('lastMessage')), users=[user_profile(user) for user in data.get('participants') or []])
...
def chat_list(data: dict) -> 'ChatList':
    ...
    if not data:
        return None
    return ChatList(chats=[chat(edge.get('node')) for edge in data.get('edges') or []], page_info=chat_page_info(data.get('pageInfo')), total_count=data.get('totalCount'))

def review(data: dict) -> 'Review':
    from .types import Review
    if not data:
        return None
    return Review(id=data.get('id'), status=ReviewStatuses.__members__.get(data.get('status')), text=data.get('text'), rating=data.get('rating'), created_at=data.get('createdAt'), updated_at=data.get('updatedAt'), deal=item_deal(data.get('deal')), creator=user_profile(data.get('creator')), moderator=moderator(data.get('moderator')), user=user_profile(data.get('user')))
...
def review_list(data: dict) -> 'ReviewList':
    ...
    if not data:
        return None
    return ReviewList(reviews=[review(edge.get('node')) for edge in data.get('edges') or []], page_info=review_page_info(data.get('pageInfo')), total_count=data.get('totalCount'))

def item_deal(data: dict) -> 'ItemDeal':
    from .types import ItemDeal
    if not data:
        return None
    return ItemDeal(id=data.get('id'), status=ItemDealStatuses.__members__.get(data.get('status')), status_expiration_date=data.get('statusExpirationDate'), status_description=data.get('statusDescription'), direction=ItemDealDirections.__members__.get(data.get('direction')), obtaining=data.get('obtaining'), has_problem=data.get('hasProblem'), report_problem_enabled=data.get('reportProblemEnabled'), completed_user=user_profile(data.get('completedBy')), props=data.get('props'), previous_status=data.get('prevStatus'), completed_at=data.get('completedAt'), created_at=data.get('createdAt'), logs=[item_log(log) for log in data.get('logs') or []], transaction=transaction(data.get('transaction')), user=user_profile(data.get('user')), chat=chat(data.get('chat')), item=item(data.get('item')), review=review(data.get('testimonial')), obtaining_fields=[game_category_data_field(field) for field in data.get('obtainingFields') or []], comment_from_buyer=data.get('commentFromBuyer'))
...
def item_deal_list(data: dict) -> 'ItemDealList':
    ...
    if not data:
        return None
    return ItemDealList(deals=[item_deal(edge.get('node')) for edge in data.get('edges') or []], page_info=item_deal_page_info(data.get('pageInfo')), total_count=data.get('totalCount'))
...
def temporary_attachment_upload_output(data: dict) -> 'TemporaryAttachmentUploadOutput':
    ...
    if not data:
        return None
    return TemporaryAttachmentUploadOutput(id=data.get('id'), url=data.get('url'), chat_id=data.get('chatId'), client_attachment_id=data.get('clientAttachmentId'), expires_at=data.get('expiresAt'))
...
def chat_message(data: dict) -> 'ChatMessage':
    ...
    if not data:
        return None
    return ChatMessage(id=data.get('id'), text=data.get('text'), created_at=data.get('createdAt'), deleted_at=data.get('deletedAt'), is_read=data.get('isRead'), is_suspicious=data.get('isSuspicious'), is_bulk_messaging=data.get('isBulkMessaging'), file=file(data.get('file')), game=game(data.get('game')), user=user_profile(data.get('user')), deal=item_deal(data.get('deal')), item=item(data.get('item')), transaction=transaction(data.get('transaction')), moderator=moderator(data.get('moderator')), event=event(data.get('event')), event_by_user=user_profile(data.get('eventByUser')), event_to_user=user_profile(data.get('eventToUser')), is_auto_response=data.get('isAutoResponse'), buttons=[chat_message_button(btn) for btn in data.get('buttons') or []])
...
def chat_message_list(data: dict) -> 'ChatMessageList':
    ...
    if not data:
        return None
    return ChatMessageList(messages=[chat_message(edge.get('node')) for edge in data.get('edges') or []], page_info=chat_message_page_info(data.get('pageInfo')), total_count=data.get('totalCount'))