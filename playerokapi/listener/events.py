...
class ChatInitializedEvent(BaseEvent):
    """
    Event: chat detected on Runner first request.
...
class NewDealEvent(BaseEvent):
    """
    Event: new deal created (buyer paid for item).
...
class DealRolledBackEvent(BaseEvent):
    """
    Event: seller refunded the deal.
...
class DealHasProblemEvent(BaseEvent):
    """
    Event: someone reported a problem with the deal.
