...
class EventListener:
    ...
    def get_message_events(
        ...
        """
Gets new message events by comparing old chats with newly fetched ones.
            ...
            for new_chat in new_chats.chats:
                ...
                if not old_chat:
                    ...
                    if new_chat.last_message.id == old_chat.last_message.id:
                        continue
                    time.sleep(2) # ебучая задержка, потому что плеерок не сразу отображает новые сообщения в запросе
                    msg_list = self.account.get_chat_messages(new_chat.id, 24)
                    new_msgs = [
    ...
    def listen(
        ...
        Sends requests in a loop to discover new events.

        :param requests_delay: Request interval (seconds).
        :type requests_delay: `int` or `float`

        :param get_new_review_events: Whether to listen for new reviews (more requests).
