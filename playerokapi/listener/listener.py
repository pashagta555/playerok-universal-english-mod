...
from ..types import ChatMessage, Chat, ItemDeal
from ..enums import ChatTypes
from ..parser import chat, chat_message
...
class EventListener:
    ...
    def _set_active_deal(self, chat: Chat, deal: ItemDeal, status_date: datetime):
        if chat.id not in self.active_deals:
            self.active_deals[chat.id] = []
        try:
            deal_tuple = [tuple for tuple in self.active_deals[chat.id] if deal.id in tuple][0]
        except:
            deal_tuple = ()
        if not deal_tuple:
    ...
    def _is_msg_processed(self, message_id: str):
        return any((msg for msg, _ in self.processed_msgs if msg.id == message_id))

    def _get_chat_processed_msgs(self, chat_id: str):
        return [msg for msg, _chat_id in self.processed_msgs if _chat_id == chat_id]

    def _parse_message_events(self, message: ChatMessage, chat: Chat) -> list[NewMessageEvent | NewDealEvent | ItemPaidEvent | ItemSentEvent | DealConfirmedEvent | DealRolledBackEvent | DealHasProblemEvent | DealProblemResolvedEvent | DealStatusChangedEvent]:
        if not message:
            return []
        if message.text == '{{ITEM_PAID}}':
            ...
            if actual_msg and actual_msg.deal:
                ...
                if deal_id not in self.processed_deals:
                    ...
                    return []
                return [NewDealEvent(actual_msg.deal, chat), ItemPaidEvent(actual_msg.deal, chat)]
        elif message.text == '{{ITEM_SENT}}':
            actual_msg = self._get_actual_message(message.id, chat.id) or message
            if actual_msg and actual_msg.deal:
                return [ItemSentEvent(actual_msg.deal, chat), DealStatusChangedEvent(actual_msg.deal, chat)]
        elif message.text == '{{DEAL_CONFIRMED}}':
            actual_msg = self._get_actual_message(message.id, chat.id) or message
            if actual_msg and actual_msg.deal:
                return [DealConfirmedEvent(actual_msg.deal, chat), DealStatusChangedEvent(actual_msg.deal, chat)]
        elif message.text == '{{DEAL_ROLLED_BACK}}':
            actual_msg = self._get_actual_message(message.id, chat.id) or message
            if actual_msg and actual_msg.deal:
                return [DealRolledBackEvent(actual_msg.deal, chat), DealStatusChangedEvent(actual_msg.deal, chat)]
        elif message.text == '{{DEAL_HAS_PROBLEM}}':
            actual_msg = self._get_actual_message(message.id, chat.id) or message
            if actual_msg and actual_msg.deal:
                return [DealHasProblemEvent(actual_msg.deal, chat), DealStatusChangedEvent(actual_msg.deal, chat)]
        elif message.text == '{{DEAL_PROBLEM_RESOLVED}}':
            actual_msg = self._get_actual_message(message.id, chat.id) or message
            if actual_msg and actual_msg.deal:
                return [DealProblemResolvedEvent(actual_msg.deal, chat), DealStatusChangedEvent(actual_msg.deal, chat)]
        return [NewMessageEvent(message, chat)]

    def _send_connection_init(self):
        self.ws.send(json.dumps({'type': 'connection_init', 'payload': {'x-gql-op': 'ws-subscription', 'x-gql-path': '/chats/[id]', 'x-timezone-offset': -180}}))

    def _subscribe_chat_updated(self):
        self.ws.send(json.dumps({'id': str(uuid.uuid4()), 'payload': {'extensions': {}, 'operationName': 'chatUpdated', 'query': QUERIES.get('chatUpdated'), 'variables': {'filter': {'userId': self.account.id}, 'showForbiddenImage': True}}, 'type': 'subscribe'}))

    def _subscribe_chat_marked_as_read(self):
        self.ws.send(json.dumps({'id': str(uuid.uuid4()), 'payload': {'extensions': {}, 'operationName': 'chatMarkedAsRead', 'query': QUERIES.get('chatMarkedAsRead'), 'variables': {'filter': {'userId': self.account.id}, 'showForbiddenImage': True}}, 'type': 'subscribe'}))

    def _subscribe_user_updated(self):
        self.ws.send(json.dumps({'id': str(uuid.uuid4()), 'payload': {'extensions': {}, 'operationName': 'userUpdated', 'query': QUERIES.get('userUpdated'), 'variables': {'userId': self.account.id}}, 'type': 'subscribe'}))
    ...
    def _process_chats_last_messages(self, chats):
        ...
        for chat in chats:
            ...
            if msg and (now - self._parse_iso(msg.created_at).astimezone(timezone.utc)).total_seconds() > 90 and (not self._is_msg_processed(msg.id)):
        ...
        try:
            ...
            if msg_data['type'] == 'connection_ack':
                ...
                if 'chatMessageCreated' in payload_data:
                    ...
                    try:
                        _chat = [chat_ for chat_ in self.chats if chat_.id == chat_id][0]
                    except:
                        return
    ...
    def listen_new_messages(self):
        headers = {'accept-encoding': 'gzip, deflate, br, zstd', 'accept-language': 'ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7', 'cache-control': 'no-cache', 'connection': 'Upgrade', 'origin': 'https://playerok.com', 'pragma': 'no-cache', 'sec-websocket-extensions': 'permessage-deflate; client_max_window_bits', 'cookie': '; '.join([f'{k}={v}' for k, v in self.account.cookies.items()]), 'user-agent': self.account.user_agent}
        ...
        try:
            self.chats = self.account.get_chats(count=24).chats
        except:
            self.chats = []
        ...
        while True:
            try:
                self.ws = websocket.WebSocket(sslopt={'ca_certs': self.account._tmp_cert_path})
                self.ws.connect(url='wss://ws.playerok.com/graphql', header=[f'{k}: {v}' for k, v in headers.items()], subprotocols=['graphql-transport-ws'], http_proxy_host=proxy_host, http_proxy_port=proxy_port, http_proxy_auth=proxy_auth)
                ...
                try:
                    ...
                    if deal.review:
                        ...
                        try:
                            ...
                            try:
                                deal.chat = self.account.get_chat(deal.chat.id)
                            except:
                                pass
                        yield NewReviewEvent(deal, deal.chat)
                except:
                    logger.debug(f'Error checking new reviews in a deal {deal_id}: {traceback.format_exc()}')
            time.sleep(1)
        ...
        while True:
            try:
                ...
                for _ in range(3):
                    try:
                        ...
                        for chat in chats:
                            ...
                            if not is_chat_processed or not is_msg_processed or (is_msg_processed and (now - self._parse_iso(last_msg.created_at).astimezone(timezone.utc)).total_seconds() <= 90):
                ...
                for chat in possible_chats:
                    last_msg = chat.last_message
                    if last_msg and last_msg.text == '{{ITEM_PAID}}' and ((now - self._parse_iso(last_msg.created_at).astimezone(timezone.utc)).total_seconds() <= 90):
                    ...
                    try:
                        ...
                        new_paid_msg = next((msg for msg in messages if msg.text == '{{ITEM_PAID}}' and (now - self._parse_iso(msg.created_at).astimezone(timezone.utc)).total_seconds() <= 90), None)
                        if new_paid_msg:
                ...
                logger.debug(f'Ошибка проверки новых deals: {traceback.format_exc()}')
            ...
            for chat_id, deals in list(self.active_deals.items()):
                for _ in range(3):
                    try:
                        ...
                        messages = sorted(msg_list.messages, key=lambda x: self._parse_iso(x.created_at))
                        break
                    except:
                        time.sleep(6)
                for deal_id, last_status, status_date in deals:
                    try:
                        status_msgs = [msg for msg in messages if self._parse_iso(msg.created_at) >= status_date and msg.deal.status]
    ...
    def listen(self, get_new_message_events: bool=True, get_new_review_events: bool=True) -> Generator[ChatInitializedEvent | NewMessageEvent | NewDealEvent | NewReviewEvent | ItemPaidEvent | ItemSentEvent | DealConfirmedEvent | DealRolledBackEvent | DealHasProblemEvent | DealProblemResolvedEvent | DealStatusChangedEvent, None, None]:
