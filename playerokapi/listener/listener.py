...
class EventListener:
    ...
    def proccess_ws_message(self, msg):
        try:
            ...
            logger.debug(f"Error handling WebSocket message: {traceback.format_exc()}")
            ...
            try:
                ...
                for chat in possible_chats:
                    ...
                    try:
                        ...
                        logger.debug(f"Error fetching message history for new chat {chat.id}: {traceback.format_exc()}")
            except websocket._exceptions.WebSocketException:
                pass
            except:
                logger.debug(f"Error checking for new deals: {traceback.format_exc()}")
            ...
            for chat_id, deals in list(self.active_deals.items()):
                ...
                for deal_id, last_status, status_date in deals:
                    try:
                        ...
                        logger.debug(f"Error checking statuses in deal {deal_id}: {traceback.format_exc()}")
                    time.sleep(8)
            time.sleep(1)
