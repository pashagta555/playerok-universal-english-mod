...
class CloudflareDetectedException(Exception):
    """
    Error indicating Cloudflare protection was detected when sending a request.
...
class RequestFailedError(Exception):
    """
    Error raised when the response status code is not 200.
...
class RequestError(Exception):
    """
    Error raised when an error occurred while sending a request.
...
class UnauthorizedError(Exception):
    ...
    def __str__(self):
        return "Failed to connect to the Playerok account. Maybe you specified an incorrect token?"
