...
class EventTypes(Enum):
    ...
    "New message in chat."
    NEW_DEAL = 2
    "A new deal has been created (when the buyer has paid for the item)."
    NEW_REVIEW = 3
    "New review from a buyer."
    DEAL_CONFIRMED = 4
    "Deal confirmed (buyer confirmed receipt of item)."
    DEAL_CONFIRMED_AUTOMATICALLY = 5
    "Deal is confirmed automatically (if the buyer does not contact for a long time)."
    DEAL_ROLLED_BACK = 6
    "Seller has issued a refund deals."
    DEAL_HAS_PROBLEM = 7
    ...
    "Deals status has been changed."
    ITEM_PAID = 10
    "The user paid for the item."
    ITEM_SENT = 11
    "Item has been sent (the seller has confirmed that the deal has been completed)."
...
class ItemLogEvents(Enum):
    ...
    PAID = 0
    "Seller confirmed the execution of the deal."
    SENT = 1
    "Item deals sent."
    DEAL_CONFIRMED = 2
    "Deal confirmed."
...
class ItemDealStatuses(Enum):
    ...
    "Deal is confirmed automatically."
    ROLLED_BACK = 5
    "Deal returned."
...
class ChatMessageButtonTypes(Enum):
    ...
    # TODO: Complete all types of message buttons
    REDIRECT = 0
    "Redirects to a link."
    LOTTERY = 1
...
class GameCategoryDataFieldInputTypes(Enum):
    ...
    INPUT = 0
    "Entered value (entered by the buyer when registering the item)."


class GameCategoryAutoConfirmPeriods(Enum):
    "Automatic confirmation periods for deals in the games category."
