...
async def callback_forms_enter_messages_page(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_mess_float_text(f'📃 Enter the page number to go to ↓'), reply_markup=main_templ.back_kb(calls.FORMS_MessagesPagination(page=last_page).pack()))
...
async def callback_forms_switch_message_enabled(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not message_id:
            raise Exception('❌ Message ID не был найден, повторите процесс с самого начала')
        ...
        if e is not TelegramAPIError:
            data = await state.get_data()
            last_page = data.get('last_page') or 0
            await throw_float_message(state=state, message=callback.message, text=templ.settings_mess_float_text(e), reply_markup=main_templ.back_kb(calls.FORMS_MessagesPagination(page=last_page).pack()))
