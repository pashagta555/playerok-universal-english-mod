...
async def handler_entering_message_text(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state,
                                  message=message,
                                  text=templ.settings_mess_page_float_text(f"✅ <b>The message text</b> for <code>{data['message_id']}</code> has been successfully changed to <blockquote>{message.text.strip()}</blockquote>"),
