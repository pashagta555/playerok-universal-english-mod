...
def menu_text ():
    txt =textwrap .dedent (f"""📝 <b>Menu{NAME }</b>

        <b>{NAME }</b> v{VERSION }A module that allows you to fill out questionnaires
...
def instruction_text ():
    txt =textwrap .dedent (f"""📖 <b>Instructions{NAME }</b>
...
def instruction_comms_text ():
    txt =textwrap .dedent (f"""📖 <b>Instructions{NAME }</b> → ⌨️ <b>Commands</b>

        <code>!myquestionnaire</code> - displays the data of the completed questionnaire
        <code>!fill in</code> - starts the process of filling out the form
...
def settings_text ():
    txt =textwrap .dedent (f"""⚙️ <b>Settings{NAME }</b>
...
def settings_mess_text ():
    ...
    txt =textwrap .dedent (f"""⚙️ <b>Settings</b> → ✉️ <b>Messages</b>
...
def settings_mess_page_text (message_id :int ):
    ...
    txt =textwrap .dedent (f"""✒️ <b>Editing message</b>

        🆔 <b>Message ID:</b>{message_id }💡 <b>State:</b>{enabled }💬 <b>Message text:</b> <blockquote>{message_text }</blockquote>
...
def settings_mess_page_kb (message_id :int ,page :int =0 ):
    ...
    message_text ="\n".join (messages [message_id ]["text"])or '❌ Not specified'
    rows =[
    [InlineKeyboardButton (text =f"💡 Condition:{enabled }",callback_data ="forms_switch_message_enabled")],
