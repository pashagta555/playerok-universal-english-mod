...
async def handle_cmds(plbot: 'PlayerokBot', event: NewMessageEvent):
    global new_forms
    if event.message.text.lower() == '!mysurvey':
        form = forms.get(event.message.user.username)
        if not form:
            plbot.send_message(event.chat.id, msg('cmd_myform_error', reason='Your questionnaire не была найдена.\nИспользуйте команду !fill out, чтобы fill out анкету.'))
            return
        plbot.send_message(event.chat.id, msg('cmd_myform', fullname=form['fullname'], age=form['age'], hobby=form['hobby']))
    elif event.message.text.lower() == '!fill out':
        new_forms[event.message.user.username] = {'fullname': '', 'age': '', 'hobby': '', 'state': 'waiting_for_fullname'}
...
async def handle_new_form_waiting_for_fullname(plbot: 'PlayerokBot', event: NewMessageEvent):
    ...
    if config['playerok']['bot']['log_states']:
        logger.info(f'{PREFIX} {Fore.LIGHTWHITE_EX}{event.message.user.username} {Fore.WHITE}indicated your full name in the application form: {Fore.LIGHTWHITE_EX}{fullname}')
...
async def handle_new_form_waiting_for_age(plbot: 'PlayerokBot', event: NewMessageEvent):
    ...
    if config['playerok']['bot']['log_states']:
        logger.info(f'{PREFIX} {Fore.LIGHTWHITE_EX}{event.message.user.username} {Fore.WHITE}indicated age in the form: {Fore.LIGHTWHITE_EX}{age}')
...
async def handle_new_form_waiting_for_hobby(plbot: 'PlayerokBot', event: NewMessageEvent):
    ...
    forms[event.message.user.username] = {'fullname': new_forms[event.message.user.username]['fullname'], 'age': new_forms[event.message.user.username]['age'], 'hobby': new_forms[event.message.user.username]['hobby']}
    del new_forms[event.message.user.username]
    if config['playerok']['bot']['log_states']:
        logger.info(f'{PREFIX} {Fore.LIGHTWHITE_EX}{event.message.user.username} {Fore.WHITE}indicated a hobby in the application form: {Fore.LIGHTWHITE_EX}{hobby}')
...
def run_in_thread_safe(func: callable, sleep_after_seconds: float=0, sleep_before_seconds: float=0):
        ...
        def check_configs_loop(cycle_delay=5):
            ...
            def _check_configs():
                ...
                if sett.get('messages') != messages:
                    messages = sett.get('messages')
                if data.get('new_forms') != new_forms:
                    data.set('new_forms', new_forms)
                if data.get('forms') != forms:
                    data.set('forms', forms)
