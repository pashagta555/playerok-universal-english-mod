...
def check_for_updates():
    try:
        ...
        if VERSION not in versions:
            logger.info(f"Your version {Fore.LIGHTWHITE_EX}{VERSION} {Fore.WHITE}нету в релизах репозитория. Последняя версия: {Fore.LIGHTWHITE_EX}{latest_release['tag_name']}")
            return
            ...
            if SKIP_UPDATES:
                logger.info(f'Пропускаю установку обновления. Если вы хотите автоматически скачивать обновления, измените значение {Fore.LIGHTWHITE_EX}SKIP_UPDATES{Fore.WHITE} on {Fore.YELLOW}False {Fore.WHITE}in the initialization file {Fore.LIGHTWHITE_EX}(__init__.py)')
                return
            logger.info(f"Downloading the update {latest_release['tag_name']}...")
