        ...
        with zipfile.ZipFile(io.BytesIO(content), 'r') as zip_ref:
            ...
            for item in os.listdir(temp_dir):
                if os.path.isdir(os.path.join(temp_dir, item)):
                    ...
                    break
            if not archive_root:
                raise Exception("Archive does not contain root folder!")
...
def check_for_updates():
    try:
        ...
        if VERSION not in versions:
            logger.info(f"Your version {Fore.LIGHTWHITE_EX}{VERSION} {Fore.WHITE}is not in repository releases. Latest version: {Fore.LIGHTWHITE_EX}{latest_release['tag_name']}")
            return
            ...
            if SKIP_UPDATES:
                logger.info(
                    ...
                    f"{Fore.LIGHTWHITE_EX}SKIP_UPDATES{Fore.WHITE} to {Fore.YELLOW}False {Fore.WHITE}in initialization file {Fore.LIGHTWHITE_EX}(__init__.py)"
                )
                return
