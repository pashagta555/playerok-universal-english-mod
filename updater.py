...
def download_update(release_info: dict) -> bytes:
    ...
    if zip_response.status_code != 200:
        raise Exception(f"Error while downloading update archive: {zip_response.status_code}")
        ...
        with zipfile.ZipFile(io.BytesIO(content), 'r') as zip_ref:
            ...
            for item in os.listdir(temp_dir):
                if os.path.isdir(os.path.join(temp_dir, item)):
                    ...
                    break
            if not archive_root:
                raise Exception("The archive does not contain a root folder!")
...
def check_for_updates():
    try:
        ...
        if VERSION not in versions:
            logger.info(f"Your version {Fore.LIGHTWHITE_EX}{VERSION} {Fore.WHITE}is not present in the repository releases. Latest version: {Fore.LIGHTWHITE_EX}{latest_release['tag_name']}")
            return
        elif Version(VERSION) == Version(latest_release["tag_name"]):
            logger.info(f"You already have the latest version installed: {Fore.LIGHTWHITE_EX}{VERSION}")
            return
        elif Version(VERSION) < Version(latest_release["tag_name"]):
            logger.info(f"{Fore.YELLOW}A new version is available: {Fore.LIGHTWHITE_EX}{latest_release['tag_name']}")
            if SKIP_UPDATES:
                logger.info(
                    f"Skipping update installation. If you want to automatically download updates, change "
            ...
            if install_update(latest_release, bytes):
                logger.info(f"{Fore.YELLOW}Update {Fore.LIGHTWHITE_EX}{latest_release['tag_name']} {Fore.YELLOW}has been successfully installed.")
                restart()
    except Exception as e:
        logger.error(f"{Fore.LIGHTRED_EX}Error while updating: {Fore.WHITE}{e}")