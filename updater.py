...
def check_for_updates():
    try:
        ...
        if VERSION not in versions:
            logger.info(f"Your version {Fore.LIGHTWHITE_EX}{VERSION} {Fore.WHITE}is not among repo releases. Latest: {Fore.LIGHTWHITE_EX}{latest_release['tag_name']}")
            return
        elif Version(VERSION) == Version(latest_release["tag_name"]):
            logger.info(f"You have the latest version: {Fore.LIGHTWHITE_EX}{VERSION}")
            return
        elif Version(VERSION) < Version(latest_release["tag_name"]):
            logger.info(f"{Fore.YELLOW}New version available: {Fore.LIGHTWHITE_EX}{latest_release['tag_name']}")
            if SKIP_UPDATES:
                logger.info(
                    f"Skipping update install. To enable auto-updates, set "
                    f"{Fore.LIGHTWHITE_EX}SKIP_UPDATES{Fore.WHITE} to {Fore.YELLOW}False {Fore.WHITE}in {Fore.LIGHTWHITE_EX}(__init__.py)"
                )
                return
            ...
            if install_update(latest_release, bytes):
                logger.info(f"{Fore.YELLOW}Update {Fore.LIGHTWHITE_EX}{latest_release['tag_name']} {Fore.YELLOW}installed successfully.")
                restart()
