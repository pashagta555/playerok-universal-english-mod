...
async def callback_delete_included_complete_deal(callback: CallbackQuery, callback_data: calls.DeleteIncludedCompleteDeal, state: FSMContext):
    try:
        ...
        index = callback_data.index
        if index is None:
            return await callback_included_complete_deals_pagination(callback, calls.IncludedRestoreItemsPagination(page=last_page), state)
...
async def callback_delete_excluded_complete_deal(callback: CallbackQuery, callback_data: calls.DeleteExcludedCompleteDeal, state: FSMContext):
    try:
        ...
        index = callback_data.index
        if index is None:
            return await callback_excluded_complete_deals_pagination(callback, calls.ExcludedRestoreItemsPagination(page=last_page), state)
        auto_complete_deals = sett.get('auto_complete_deals')
...
async def callback_remember_username(callback: CallbackQuery, callback_data: calls.RememberUsername, state: FSMContext):
    ...
    if do == 'send_mess':
        ...
        await throw_float_message(state=state, message=callback.message, text=templ.do_action_text(f'💬 Введите <b>сообщение</b> для отправки <b>{username}</b>:'), reply_markup=templ.destroy_kb(), callback=callback, send=True)
...
async def callback_remember_deal_id(callback: CallbackQuery, callback_data: calls.RememberDealId, state: FSMContext):
    ...
    if do == 'refund':
        await throw_float_message(state=state, message=callback.message, text=templ.do_action_text(f'📦✔️ Подтвердите <b>возврат</b> <a href="https://playerok.com/deal/{deal_id}">сделки</a>:'), reply_markup=templ.confirm_kb(confirm_cb='refund_deal', cancel_cb='destroy'), callback=callback, send=True)
    if do == 'complete':
        await throw_float_message(state=state, message=callback.message, text=templ.do_action_text(f'☑️✔️ Подтвердите <b>выполнение</b> <a href="https://playerok.com/deal/{deal_id}">сделки</a>:'), reply_markup=templ.confirm_kb(confirm_cb='complete_deal', cancel_cb='destroy'), callback=callback, send=True)
...
async def callback_select_sbp_bank(callback: CallbackQuery, callback_data: calls.SelectSbpBank, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_withdrawal_sbp_float_text(f'📲 Введите <b>номер телефона</b>, на который нужно будет совершать вывод:'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='withdrawal').pack()))
...
async def callback_set_new_deliv_piece(callback: CallbackQuery, callback_data: calls.SetNewDelivPiece, state: FSMContext):
    ...
    if value:
        ...
        await throw_float_message(state=state, message=callback.message, text=templ.settings_new_deliv_float_text(f'💬 Введите <b>сообщение авто-выдачи</b>, которое будет отправляться после покупки товара:'), reply_markup=templ.back_kb(calls.AutoDeliveriesPagination(page=last_page).pack()), callback=callback)
...
async def callback_refund_deal(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.do_action_text(f'✅ По сделке <b>https://playerok.com/deal/{deal_id}</b> был оформлен возврат'), reply_markup=templ.destroy_kb())
...
async def callback_complete_deal(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.do_action_text(f'✅ Сделка <b>https://playerok.com/deal/{deal_id}</b> была помечена вами, как выполненная'), reply_markup=templ.destroy_kb())

@router.callback_query(F.data == 'bump_items')
async def callback_bump_items(callback: CallbackQuery, state: FSMContext):
    try:
        await state.set_state(None)
        await throw_float_message(state=state, message=callback.message, text=templ.events_float_text(f'⬆️ Идёт <b>поднятие предметов</b>, ожидайте (см. консоль)...'), reply_markup=templ.back_kb(calls.MenuNavigation(to='events').pack()))
        from plbot.playerokbot import get_playerok_bot
        get_playerok_bot().bump_items()
        await throw_float_message(state=state, message=callback.message, text=templ.events_float_text(f'⬆️✅ <b>Предметы</b> были успешно подняты'), reply_markup=templ.back_kb(calls.MenuNavigation(to='events').pack()))
    except Exception as e:
        await throw_float_message(state=state, message=callback.message, text=templ.events_float_text(e), reply_markup=templ.back_kb(calls.MenuNavigation(to='events').pack()))
...
async def callback_request_withdrawal(callback: CallbackQuery, state: FSMContext):
    try:
        await state.set_state(None)
        await throw_float_message(state=state, message=callback.message, text=templ.events_float_text(f'💸 Создаю <b>транзакцию на вывод средств</b>, ожидайте (см. консоль)...'), reply_markup=templ.back_kb(calls.MenuNavigation(to='events').pack()))
        from plbot.playerokbot import get_playerok_bot
        success = get_playerok_bot().request_withdrawal()
        if success:
            await throw_float_message(state=state, message=callback.message, text=templ.events_float_text(f'✅ <b>Транзакция на вывод средств</b> была успешно создана'), reply_markup=templ.back_kb(calls.MenuNavigation(to='events').pack()))
        else:
            await throw_float_message(state=state, message=callback.message, text=templ.events_float_text(f'❌ Не удалось создать <b>транзакцию на вывод средств</b> (см. консоль на наличие ошибок)'), reply_markup=templ.back_kb(calls.MenuNavigation(to='events').pack()))
    except Exception as e:
        await throw_float_message(state=state, message=callback.message, text=templ.events_float_text(e), reply_markup=templ.back_kb(calls.MenuNavigation(to='events').pack()))
...
async def callback_send_new_included_restore_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_restore_included_float_text('📄 Отправьте <b>.txt</b> файл с <b>ключевыми фразами</b>, по одной записи в строке (для каждого товара указываются через запятую, например, "samp аккаунт, со всеми данными")'), reply_markup=templ.back_kb(calls.IncludedRestoreItemsPagination(page=last_page).pack()))
...
async def callback_send_new_excluded_restore_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_restore_excluded_float_text('📄 Отправьте <b>.txt</b> файл с <b>ключевыми фразами</b>, по одной записи в строке (для каждого товара указываются через запятую, например, "samp аккаунт, со всеми данными")'), reply_markup=templ.back_kb(calls.ExcludedRestoreItemsPagination(page=last_page).pack()))
...
async def callback_send_new_included_complete_deals_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_complete_included_float_text('📄 Отправьте <b>.txt</b> файл с <b>ключевыми фразами</b>, по одной записи в строке (для каждого товара указываются через запятую, например, "samp аккаунт, со всеми данными")'), reply_markup=templ.back_kb(calls.IncludedCompleteDealsPagination(page=last_page).pack()))
...
async def callback_send_new_excluded_complete_deals_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_complete_excluded_float_text('📄 Отправьте <b>.txt</b> файл с <b>ключевыми фразами</b>, по одной записи в строке (для каждого товара указываются через запятую, например, "samp аккаунт, со всеми данными")'), reply_markup=templ.back_kb(calls.ExcludedCompleteDealsPagination(page=last_page).pack()))
...
async def callback_send_new_included_bump_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_bump_included_float_text('📄 Отправьте <b>.txt</b> файл с <b>ключевыми фразами</b>, по одной записи в строке (для каждого товара указываются через запятую, например, "samp аккаунт, со всеми данными")'), reply_markup=templ.back_kb(calls.IncludedBumpItemsPagination(page=last_page).pack()))
...
async def callback_send_new_excluded_bump_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_bump_excluded_float_text('📄 Отправьте <b>.txt</b> файл с <b>ключевыми фразами</b>, по одной записи в строке (для каждого товара указываются через запятую, например, "samp аккаунт, со всеми данными")'), reply_markup=templ.back_kb(calls.ExcludedBumpItemsPagination(page=last_page).pack()))
...
async def callback_add_new_custom_command(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=callback.message, text=templ.settings_new_comm_float_text(f'✅ <b>Команда</b> <code>{command}</code> была успешно добавлена'), reply_markup=templ.back_kb(calls.CustomCommandsPagination(page=last_page).pack()))
    except Exception as e:
        await throw_float_message(state=state, message=callback.message, text=templ.settings_new_comm_float_text(e), reply_markup=templ.back_kb(calls.CustomCommandsPagination(page=last_page).pack()))
...
async def callback_confirm_deleting_custom_command(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not command:
            return await callback_custom_commands_pagination(callback, calls.CustomCommandsPagination(page=last_page), state)
        await throw_float_message(state=state, message=callback.message, text=templ.settings_comm_page_float_text(f'🗑️ Подтвердите <b>удаление команды</b> <code>{command}</code>'), reply_markup=templ.confirm_kb(confirm_cb='delete_custom_command', cancel_cb=calls.CustomCommandPage(command=command).pack()))
    except Exception as e:
...
async def callback_delete_custom_command(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        sett.set('custom_commands', custom_commands)
        await throw_float_message(state=state, message=callback.message, text=templ.settings_comm_page_float_text(f'✅ <b>Команда</b> <code>{command}</code> была удалена'), reply_markup=templ.back_kb(calls.CustomCommandsPagination(page=last_page).pack()))
    except Exception as e:
        await throw_float_message(state=state, message=callback.message, text=templ.settings_comm_page_float_text(e), reply_markup=templ.back_kb(calls.CustomCommandsPagination(page=last_page).pack()))
...
async def callback_confirm_deleting_auto_delivery(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if index is None:
            return await callback_auto_deliveries_pagination(callback, calls.AutoDeliveriesPagination(page=last_page), state)
        await throw_float_message(state=state, message=callback.message, text=templ.settings_deliv_page_float_text('🗑️ Подтвердите <b>удаление авто-выдачи</b>:'), reply_markup=templ.confirm_kb(confirm_cb='delete_auto_delivery', cancel_cb=calls.AutoDeliveryPage(index=index).pack()))
    except Exception as e:
...
async def callback_delete_auto_delivery(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        sett.set('auto_deliveries', auto_deliveries)
        await throw_float_message(state=state, message=callback.message, text=templ.settings_deliv_page_float_text('✅ <b>Авто-выдача</b> была удалена'), reply_markup=templ.back_kb(calls.AutoDeliveriesPagination(page=last_page).pack()))
    except Exception as e:
        await throw_float_message(state=state, message=callback.message, text=templ.settings_deliv_page_float_text(e), reply_markup=templ.back_kb(calls.AutoDeliveriesPagination(page=last_page).pack()))
...
async def callback_reload_module(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not uuid:
            return await callback_modules_pagination(callback, calls.ModulePage(page=last_page), state)
        from core.modules import reload_module
        await reload_module(uuid)
        return await callback_module_page(callback, calls.ModulePage(uuid=uuid), state)
    except Exception as e:
        await throw_float_message(state=state, message=callback.message, text=templ.module_page_float_text(e), reply_markup=templ.back_kb(calls.ModulesPagination(page=last_page).pack()))
...
async def callback_select_logs_file_lines(callback: CallbackQuery, state: FSMContext):
    await state.set_state(None)
    await throw_float_message(state=state, message=callback.message, text=templ.logs_float_text('Select file size:'), reply_markup=templ.logs_file_lines_kb())
