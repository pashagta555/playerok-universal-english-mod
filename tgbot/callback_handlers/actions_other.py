...
async def callback_delete_included_restore_item(callback: CallbackQuery, callback_data: calls.DeleteIncludedRestoreItem, state: FSMContext):
    try:
        ...
        if index is None:
            raise Exception("❌ Included item was not found, please restart the process from the beginning")
...
async def callback_delete_excluded_restore_item(callback: CallbackQuery, callback_data: calls.DeleteExcludedRestoreItem, state: FSMContext):
    try:
        ...
        index = callback_data.index
        if index is None:
            raise Exception("❌ Excluded item was not found, please restart the process from the beginning")
...
async def callback_delete_included_bump_item(callback: CallbackQuery, callback_data: calls.DeleteIncludedBumpItem, state: FSMContext):
    try:
        ...
        index = callback_data.index
        if index is None:
            raise Exception("❌ Failed to find item, please restart the process from the beginning")
...
async def callback_remember_username(callback: CallbackQuery, callback_data: calls.RememberUsername, state: FSMContext):
    ...
    if do == "send_mess":
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.do_action_text(f"💬 Enter a <b>message</b> to send to <b>{username}</b> ↓"), 
...
async def callback_remember_deal_id(callback: CallbackQuery, callback_data: calls.RememberDealId, state: FSMContext):
    ...
    if do == "refund":
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.do_action_text(f'📦✔️ Confirm the <b>refund</b> for the <a href="https://playerok.com/deal/{deal_id}">deal</a> ↓'), 
    ...
    if do == "complete":
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.do_action_text(f'☑️✔️ Confirm <b>completion</b> for the <a href="https://playerok.com/deal/{deal_id}">deal</a> ↓'), 
...
async def callback_select_sbp_bank(callback: CallbackQuery, callback_data: calls.SelectSbpBank, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_withdrawal_sbp_float_text(f"📲 Enter the <b>phone number</b> to withdraw to:"), 
...
async def callback_refund_deal(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.do_action_text(f"✅ A refund has been issued for deal <b>https://playerok.com/deal/{deal_id}</b>"), 
        reply_markup=templ.destroy_kb()
    )
...
async def callback_complete_deal(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.do_action_text(f"✅ Deal <b>https://playerok.com/deal/{deal_id}</b> was marked as completed"), 
        reply_markup=templ.destroy_kb()
    )
...
async def callback_bump_items(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.events_float_text(f"⬆️ <b>Item bumping</b> in progress, please wait (see console)..."), 
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.events_float_text(f"⬆️✅ <b>Items</b> were successfully bumped"), 
            reply_markup=templ.back_kb(calls.MenuNavigation(to="events").pack())
        )
    except Exception as e:
        await throw_float_message(
...
async def callback_request_withdrawal(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.events_float_text(f"💸 Creating a <b>withdrawal transaction</b>, please wait (see console)..."), 
            reply_markup=templ.back_kb(calls.MenuNavigation(to="events").pack())
        )
        ...
        if success:
            await throw_float_message(
                state=state, 
                message=callback.message, 
                text=templ.events_float_text(f"💸✅ <b>Withdrawal transaction</b> was successfully created"), 
                reply_markup=templ.back_kb(calls.MenuNavigation(to="events").pack())
            )
        else:
            await throw_float_message(
                state=state, 
                message=callback.message, 
                text=templ.events_float_text(f"💸❌ Failed to create a <b>withdrawal transaction</b> (see console for errors)"), 
                reply_markup=templ.back_kb(calls.MenuNavigation(to="events").pack())
            )
    except Exception as e:
        await throw_float_message(
...
async def callback_send_new_included_restore_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_new_restore_included_float_text(f"📄 Send a <b>.txt</b> file with <b>keyphrases</b>, one entry per line (comma-separated per item, e.g. \"samp account, with all data\")"), 
        reply_markup=templ.back_kb(calls.IncludedRestoreItemsPagination(page=last_page).pack())
    )
...
async def callback_send_new_excluded_restore_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_new_restore_excluded_float_text(f"📄 Send a <b>.txt</b> file with <b>keyphrases</b>, one entry per line (comma-separated per item, e.g. \"samp account, with all data\")"), 
...
async def callback_send_new_included_bump_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_new_bump_included_float_text(f"📄 Send a <b>.txt</b> file with <b>keyphrases</b>, one entry per line (comma-separated per item, e.g. \"samp account, with all data\")"), 
        reply_markup=templ.back_kb(calls.IncludedBumpItemsPagination(page=last_page).pack())
    )
...
async def callback_send_new_excluded_bump_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_new_bump_excluded_float_text(f"📄 Send a <b>.txt</b> file with <b>keyphrases</b>, one entry per line (comma-separated per item, e.g. \"samp account, with all data\")"), 
        reply_markup=templ.back_kb(calls.ExcludedBumpItemsPagination(page=last_page).pack())
    )
...
async def callback_add_new_custom_command(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not new_custom_command:
            raise Exception("❌ New command was not found, please restart the process from the beginning")
        if not new_custom_command_answer:
            raise Exception("❌ New command reply was not found, please restart the process from the beginning")
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.settings_new_comm_float_text(f"✅ <b>Command</b> <code>{new_custom_command}</code> was added"), 
...
async def callback_confirm_deleting_custom_command(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not custom_command:
            raise Exception("❌ Command was not found, please restart the process from the beginning")
        
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.settings_comm_page_float_text(f"🗑️ Confirm <b>command deletion</b> <code>{custom_command}</code>"), 
...
async def callback_delete_custom_command(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.settings_comm_page_float_text(f"✅ <b>Command</b> <code>{custom_command}</code> was deleted"), 
            reply_markup=templ.back_kb(calls.CustomCommandsPagination(page=last_page).pack())
        )
    except Exception as e:
        data = await state.get_data()
...
async def callback_add_new_auto_delivery(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not new_auto_delivery_keyphrases:
            raise Exception("❌ Auto-delivery keyphrases were not found, please restart the process from the beginning")
        if not new_auto_delivery_message:
            raise Exception("❌ Auto-delivery message was not found, please restart the process from the beginning")
...
async def callback_confirm_deleting_auto_delivery(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if auto_delivery_index is None:
            raise Exception("❌ Auto-delivery was not found, please restart the process from the beginning")
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.settings_deliv_page_float_text(f"🗑️ Confirm <b>auto-delivery deletion</b> for keyphrases <code>{auto_delivery_keyphrases}</code>"), 
...
async def callback_reload_module(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if not module_uuid:
            raise Exception("❌ Module UUID was not found, please restart the process from the beginning")
...
async def callback_select_logs_file_lines(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.logs_float_text(f"Choose file size:"), 
