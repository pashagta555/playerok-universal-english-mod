...
async def callback_delete_included_restore_item(callback: CallbackQuery, callback_data: calls.DeleteIncludedRestoreItem, state: FSMContext):
    try:
        ...
        if index is None:
            raise Exception("❌ Included item was not found, restart the process from the beginning")
...
async def callback_delete_excluded_restore_item(callback: CallbackQuery, callback_data: calls.DeleteExcludedRestoreItem, state: FSMContext):
    try:
        ...
        index = callback_data.index
        if index is None:
            raise Exception("❌ Excluded item was not found, restart the process from the beginning")
...
async def callback_delete_included_bump_item(callback: CallbackQuery, callback_data: calls.DeleteIncludedBumpItem, state: FSMContext):
    try:
        ...
        index = callback_data.index
        if index is None:
            raise Exception("❌ Included item not found, please restart the process from the beginning")
...
async def callback_delete_excluded_bump_item(callback: CallbackQuery, callback_data: calls.DeleteExcludedBumpItem, state: FSMContext):
    try:
        ...
        index = callback_data.index
        if index is None:
            raise Exception("❌ Excluded item not found, please restart the process from the beginning")
...
async def callback_remember_deal_id(callback: CallbackQuery, callback_data: calls.RememberDealId, state: FSMContext):
    ...
    if do == "refund":
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.do_action_text(f'📦✔️ Confirm <b>refund</b> for <a href="https://playerok.com/deal/{deal_id}">deal</a> ↓'), 
...
async def callback_bump_items(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.events_float_text(f"⬆️ <b>Bumping items</b> in progress, please wait (see console)..."), 
...
async def callback_send_new_included_bump_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_new_bump_included_float_text(f"📄 Send <b>.txt</b> file with <b>key phrases</b>, one entry per line (for each item, separated by commas, e.g., \"samp account, with all data\")"), 
        reply_markup=templ.back_kb(calls.IncludedBumpItemsPagination(page=last_page).pack())
    )
...
async def callback_send_new_excluded_bump_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_new_bump_excluded_float_text(f"📄 Send <b>.txt</b> file with <b>key phrases</b>, one entry per line (for each item, separated by commas, e.g., \"samp account, with all data\")"), 
        reply_markup=templ.back_kb(calls.ExcludedBumpItemsPagination(page=last_page).pack())
    )
...
async def callback_add_new_custom_command(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not new_custom_command:
            raise Exception("❌ New command was not found, restart the process from the beginning")
        if not new_custom_command_answer:
            raise Exception("❌ Response for new command was not found, restart the process from the beginning")
...
async def callback_confirm_deleting_custom_command(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not custom_command:
            raise Exception("❌ Command was not found, restart the process from the beginning")
        
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.settings_comm_page_float_text(f"🗑️ Confirm <b>deleting command</b> <code>{custom_command}</code>"), 
...
async def callback_add_new_auto_delivery(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not new_auto_delivery_keyphrases:
            raise Exception("❌ Auto-delivery key phrases were not found, restart the process from the beginning")
        if not new_auto_delivery_message:
            raise Exception("❌ Auto-delivery message was not found, restart the process from the beginning")
...
async def callback_confirm_deleting_auto_delivery(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if auto_delivery_index is None:
            raise Exception("❌ Auto-delivery was not found, restart the process from the beginning")
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.settings_deliv_page_float_text(f"🗑️ Confirm <b>deleting auto-delivery</b> for key phrases <code>{auto_delivery_keyphrases}</code>"), 
...
async def callback_reload_module(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if not module_uuid:
            raise Exception("❌ Module UUID was not found, restart the process from the beginning")
