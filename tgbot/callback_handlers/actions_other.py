...
async def callback_delete_included_restore_item(callback: CallbackQuery, callback_data: calls.DeleteIncludedRestoreItem, state: FSMContext):
    try:
        ...
        if index is None:
            raise Exception("❌ Included item was not found, please start over")
...
async def callback_delete_excluded_restore_item(callback: CallbackQuery, callback_data: calls.DeleteExcludedRestoreItem, state: FSMContext):
    try:
        ...
        index = callback_data.index
        if index is None:
            raise Exception("❌ Excluded item was not found, please start over")
...
async def callback_delete_included_bump_item(callback: CallbackQuery, callback_data: calls.DeleteIncludedBumpItem, state: FSMContext):
    try:
        ...
        index = callback_data.index
        if index is None:
            raise Exception("❌ Failed to find item, please start over")
...
async def callback_remember_username(callback: CallbackQuery, callback_data: calls.RememberUsername, state: FSMContext):
    ...
    if do == "send_mess":
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.do_action_text(f"💬 Enter <b>message</b> to send <b>{username}</b> ↓"), 
...
async def callback_remember_deal_id(callback: CallbackQuery, callback_data: calls.RememberDealId, state: FSMContext):
    ...
    if do == "refund":
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.do_action_text(f'📦✔️ Confirm <b>refund</b> <a href="https://playerok.com/deal/{deal_id}">deal</a> ↓'), 
    ...
    if do == "complete":
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.do_action_text(f'☑️✔️ Confirm <b>completion</b> <a href="https://playerok.com/deal/{deal_id}">deal</a> ↓'), 
...
async def callback_select_sbp_bank(callback: CallbackQuery, callback_data: calls.SelectSbpBank, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_withdrawal_sbp_float_text(f"📲 Enter <b>phone number</b>, for withdrawals:"), 
...
async def callback_refund_deal(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.do_action_text(f"✅ Deal <b>https://playerok.com/deal/{deal_id}</b> was refunded"), 
        reply_markup=templ.destroy_kb()
    )
...
async def callback_bump_items(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.events_float_text(f"⬆️ Bumping <b>items</b>, please wait (see console)..."), 
...
async def callback_request_withdrawal(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.events_float_text(f"💸 Creating <b>withdrawal transaction</b>, please wait (see console)..."), 
            reply_markup=templ.back_kb(calls.MenuNavigation(to="events").pack())
        )
        ...
        if success:
            await throw_float_message(
                state=state, 
                message=callback.message, 
                text=templ.events_float_text(f"💸✅ <b>Withdrawal transaction</b> was created successfully"), 
                reply_markup=templ.back_kb(calls.MenuNavigation(to="events").pack())
            )
        else:
            await throw_float_message(
                state=state, 
                message=callback.message, 
                text=templ.events_float_text(f"💸❌ Failed to create <b>withdrawal transaction</b> (see console for errors)"), 
                reply_markup=templ.back_kb(calls.MenuNavigation(to="events").pack())
            )
    except Exception as e:
        await throw_float_message(
...
async def callback_send_new_included_restore_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_new_restore_included_float_text(f"📄 Send a <b>.txt</b> file with <b>key phrases</b>, one per line (for each item, comma-separated, e.g. \"samp account, with all data\")"), 
        reply_markup=templ.back_kb(calls.IncludedRestoreItemsPagination(page=last_page).pack())
    )
...
async def callback_send_new_excluded_restore_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_new_restore_excluded_float_text(f"📄 Send a <b>.txt</b> file with <b>key phrases</b>, one per line (for each item, comma-separated, e.g. \"samp account, with all data\")"), 
...
async def callback_send_new_included_bump_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_new_bump_included_float_text(f"📄 Send a <b>.txt</b> file with <b>key phrases</b>, one per line (for each item, comma-separated, e.g. \"samp account, with all data\")"), 
        reply_markup=templ.back_kb(calls.IncludedBumpItemsPagination(page=last_page).pack())
    )
...
async def callback_send_new_excluded_bump_items_keyphrases_file(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_new_bump_excluded_float_text(f"📄 Send a <b>.txt</b> file with <b>key phrases</b>, one per line (for each item, comma-separated, e.g. \"samp account, with all data\")"), 
        reply_markup=templ.back_kb(calls.ExcludedBumpItemsPagination(page=last_page).pack())
    )
...
async def callback_add_new_custom_command(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not new_custom_command:
            raise Exception("❌ New command was not found, please start over")
        if not new_custom_command_answer:
            raise Exception("❌ Reply to new command was not found, please start over")
...
async def callback_confirm_deleting_custom_command(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not custom_command:
            raise Exception("❌ Command was not found, please start over")
...
async def callback_add_new_auto_delivery(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not new_auto_delivery_keyphrases:
            raise Exception("❌ Auto-delivery key phrases were not found, please start over")
        if not new_auto_delivery_message:
            raise Exception("❌ Auto-delivery message was not found, please start over")
...
async def callback_confirm_deleting_auto_delivery(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.settings_deliv_page_float_text(f"🗑️ Confirm <b>auto-delivery deletion</b> for key phrases <code>{auto_delivery_keyphrases}</code>"), 
...
async def callback_reload_module(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if not module_uuid:
            raise Exception("❌ Module UUID was not found, please start over")
