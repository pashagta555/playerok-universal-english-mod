...
async def callback_remember_username(callback: CallbackQuery, callback_data: calls.RememberUsername, state: FSMContext):
    ...
    if do == "send_mess":
        ...
        await throw_float_message(
            state=state,
            message=callback.message,
            text=templ.do_action_text(f"💬 Enter a <b>message</b> to send <b>{username}</b>:"),
...
async def callback_remember_deal_id(callback: CallbackQuery, callback_data: calls.RememberDealId, state: FSMContext):
    ...
    if do == "refund":
        await throw_float_message(
            state=state,
            message=callback.message,
            text=templ.do_action_text(f'📦✔️ Confirm <b>return</b> <a href="https://playerok.com/deal/{deal_id}">transactions</a>:'),
    ...
    if do == "complete":
        await throw_float_message(
            state=state,
            message=callback.message,
            text=templ.do_action_text(f'☑️✔️ Confirm <b>execution</b> <a href="https://playerok.com/deal/{deal_id}">deal</a>:'),
...
async def callback_select_bank_card(callback: CallbackQuery, callback_data: calls.SelectBankCard, state: FSMContext):
    ...
    config["player"]["auto_withdrawal"]["credentials_type"] = "card"
    config["player"]["auto_withdrawal"]["card_id"] = card_id
...
async def callback_select_sbp_bank(callback: CallbackQuery, callback_data: calls.SelectSbpBank, state: FSMContext):
    ...
    await throw_float_message(
        state=state,
        message=callback.message,
        text=templ.settings_withdrawal_sbp_float_text(f"📲 Enter <b>phone number</b> to which you want to make withdrawals:"),
...
async def callback_set_new_deliv_piece(callback: CallbackQuery, callback_data: calls.SetNewDelivPiece, state: FSMContext):
    ...
    if value:
        await state.set_state(states.AutoDeliveryStates.waiting_for_new_auto_delivery_goods)
        await throw_float_message(
            state=state,
            message=callback.message,
            text=templ.settings_new_deliv_float_text(
                f"📦 Send <b>products</b> for individual delivery (1 line = 1 product, you can send a .txt file with the products):"
            ),
            reply_markup=templ.back_kb(calls.AutoDeliveriesPagination(page=last_page).pack()),
            callback=callback
        )
    else:
        await state.set_state(states.AutoDeliveryStates.waiting_for_new_auto_delivery_message)
        await throw_float_message(
            state=state,
            message=callback.message,
            text=templ.settings_new_deliv_float_text(
                f"💬 Enter the <b>auto-delivery message</b> that will be sent after purchasing the product:"
            ),
            reply_markup=templ.back_kb(calls.AutoDeliveriesPagination(page=last_page).pack()),
            callback=callback
        )
...
async def callback_refund_deal(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state,
        message=callback.message,
        text=templ.do_action_text(f"✅ A refund was issued for the transaction <b>https://playerok.com/deal/{deal_id}</b>"),
        reply_markup=templ.destroy_kb()
    )
...
async def callback_complete_deal(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state,
        message=callback.message,
        text=templ.do_action_text(f"✅ The transaction <b>https://playerok.com/deal/{deal_id}</b> has been marked by you as completed"),
        reply_markup=templ.destroy_kb()
    )
...
async def callback_bump_items(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=callback.message,
            text=templ.events_float_text(f"⬆️ <b>raising items</b> is in progress, please wait (see console)..."),
        ...
        await throw_float_message(
            state=state,
            message=callback.message,
            text=templ.events_float_text(f"⬆️✅ <b>Items</b> were successfully picked up"),
            reply_markup=templ.back_kb(calls.MenuNavigation(to="events").pack())
        )
    except Exception as e:
        await throw_float_message(
...
async def callback_request_withdrawal(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=callback.message,
            text=templ.events_float_text(f"💸 I am creating a <b>transaction to withdraw funds</b>, please wait (see console)..."),
            reply_markup=templ.back_kb(calls.MenuNavigation(to="events").pack())
        )
        ...
        if success:
            ...
            await throw_float_message(
                state=state,
                message=callback.message,
                text=templ.events_float_text(f"❌ Failed to create <b>withdrawal transaction</b> (check the console for errors)"),
                reply_markup=templ.back_kb(calls.MenuNavigation(to="events").pack())
            )
    except Exception as e:
        await throw_float_message(
...
async def callback_clean_tg_logging_chat_id(callback: CallbackQuery, state: FSMContext):
    ...
    config = sett.get("config")
    config["player"]["tg_logging"]["chat_id"] = ""
    sett.set("config", config)
...
async def callback_add_new_custom_command(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=callback.message,
            text=templ.settings_new_comm_float_text(f"✅ <b>Command</b> <code>{command}</code> was added successfully"),
...
async def callback_confirm_deleting_custom_command(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=callback.message,
            text=templ.settings_comm_page_float_text(f"🗑️ Confirm <b>deleting command</b> <code>{command}</code>"),
...
async def callback_delete_custom_command(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=callback.message,
            text=templ.settings_comm_page_float_text(f"✅ <b>Command</b> <code>{command}</code> has been deleted"),
            reply_markup=templ.back_kb(calls.CustomCommandsPagination(page=last_page).pack())
        )
    except Exception as e:
        await throw_float_message(
...
async def callback_add_new_auto_delivery(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=callback.message,
            text=templ.settings_new_deliv_float_text(f"✅ <b>Auto-deliver</b> was added successfully"),
            reply_markup=templ.back_kb(calls.AutoDeliveriesPagination(page=last_page).pack())
        )
    except Exception as e:
        await throw_float_message(
