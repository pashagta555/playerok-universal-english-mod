...
async def callback_remember_deal_id (callback :CallbackQuery ,callback_data :calls .RememberDealId ,state :FSMContext ):
    ...
    if do =="complete":
        ...
        state =state ,
        message =callback .message ,
        text =templ .do_action_text (f'☑️✔️ Confirm <b>execution</b> <a href="https://playerok.com/deal/{deal_id }">transactions</a>:'),
...
async def callback_select_sbp_bank (callback :CallbackQuery ,callback_data :calls .SelectSbpBank ,state :FSMContext ):
    ...
    state =state ,
    message =callback .message ,
    text =templ .settings_withdrawal_sbp_float_text (f"📲 Enter <b>phone number</b> to which you will need to make a withdrawal:"),
...
async def callback_set_new_deliv_piece (callback :CallbackQuery ,callback_data :calls .SetNewDelivPiece ,state :FSMContext ):
    ...
    if value :
        ...
        message =callback .message ,
        text =templ .settings_new_deliv_float_text (
        f"💬 Enter <b>auto-delivery message</b>, which will be sent after purchasing the product:"
        ),
        reply_markup =templ .back_kb (calls .AutoDeliveriesPagination (page =last_page ).pack ()),
        callback =callback 
        )
...
async def callback_refund_deal (callback :CallbackQuery ,state :FSMContext ):
    ...
    state =state ,
    message =callback .message ,
    text =templ .do_action_text (f"✅ According to the deal <b>https://playerok.com/deal/{deal_id }</b> refund was issued"),
    reply_markup =templ .destroy_kb ()
    )
...
async def callback_complete_deal (callback :CallbackQuery ,state :FSMContext ):
    ...
    state =state ,
    message =callback .message ,
    text =templ .do_action_text (f"✅ Deal <b>https://playerok.com/deal/{deal_id }</b> has been marked by you as completed"),
    reply_markup =templ .destroy_kb ()
    )
...
async def callback_bump_items (callback :CallbackQuery ,state :FSMContext ):
    try :
        ...
        state =state ,
        message =callback .message ,
        text =templ .events_float_text (f"⬆️ <b>raising items</b> is in progress, wait (see console)..."),
...
async def callback_request_withdrawal (callback :CallbackQuery ,state :FSMContext ):
    try :
        ...
        state =state ,
        message =callback .message ,
        text =templ .events_float_text (f"💸 I am creating <b>transaction to withdraw funds</b>, please wait (see console)..."),
        reply_markup =templ .back_kb (calls .MenuNavigation (to ="events").pack ())
        )
...
async def callback_send_new_included_restore_items_keyphrases_file (callback :CallbackQuery ,state :FSMContext ):
    ...
    '📄 Send <b>.txt</b> file with <b>key phrases</b>, one entry per line'
...
async def callback_add_new_custom_command (callback :CallbackQuery ,state :FSMContext ):
    try :
        ...
        state =state ,
        message =callback .message ,
        text =templ .settings_new_comm_float_text (f"✅ <b>Team</b> <code>{command }</code> was added successfully"),
...
async def callback_delete_custom_command (callback :CallbackQuery ,state :FSMContext ):
    try :
        ...
        state =state ,
        message =callback .message ,
        text =templ .settings_comm_page_float_text (f"✅ <b>Team</b> <code>{command }</code> has been removed"),
        reply_markup =templ .back_kb (calls .CustomCommandsPagination (page =last_page ).pack ())
        )
    except Exception as e :
        await throw_float_message (
...
async def callback_add_new_auto_delivery (callback :CallbackQuery ,state :FSMContext ):
    try :
        ...
        state =state ,
        message =callback .message ,
        text =templ .settings_new_deliv_float_text (f"✅ <b>Auto-dispense</b> has been successfully added"),
        reply_markup =templ .back_kb (calls .AutoDeliveriesPagination (page =last_page ).pack ())
        )
    except Exception as e :
        await throw_float_message (
