...
async def callback_remember_username(callback: CallbackQuery, callback_data: calls.RememberUsername, state: FSMContext):
    ...
    if do == "send_mess":
        ...
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.do_action_text(f"💬 Enter <b>message</b> to be send to <b>{username}</b> ↓"), 
...
async def callback_remember_deal_id(callback: CallbackQuery, callback_data: calls.RememberDealId, state: FSMContext):
    ...
    if do == "refund":
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.do_action_text(f'📦✔️ Confirm <b>refund</b> <a href="https://playerok.com/deal/{deal_id}">of the deal</a> ↓'), 
    ...
    if do == "complete":
        await throw_float_message(
            state=state, 
            message=callback.message, 
            text=templ.do_action_text(f'☑️✔️ Confirm <b>accompletition</b> <a href="https://playerok.com/deal/{deal_id}">of the deal</a> ↓'), 
