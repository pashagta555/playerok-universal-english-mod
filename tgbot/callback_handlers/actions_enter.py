...
async def callback_enter_cookies(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_auth_float_text(f'🍪 Введите новые <b>Cookie-данные</b> вашего аккаунта:\n・ Текущие: <blockquote>{cookies}</blockquote>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='auth').pack()))
...
async def callback_enter_pl_proxy(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_conn_float_text(f'🌐 Введите новый <b>прокси для FunPay</b> (формат: user:pass@ip:port или ip:port):\n・ Текущий: <code>{proxy}</code>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='conn').pack()))
...
async def callback_enter_tg_proxy(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_conn_float_text(f'🌐 Введите новый <b>прокси для Telegram</b> (формат: user:pass@ip:port или ip:port):\n・ Текущий: <code>{proxy}</code>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='conn').pack()))
...
async def callback_enter_requests_timeout(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_conn_float_text(f'🛜 Введите новый <b>таймаут подключения</b> (в секундах):\n・ Текущее: <code>{requests_timeout}</code>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='conn').pack()))
...
async def callback_enter_watermark_value(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_other_float_text(f'✍️©️ Введите новый <b>водяной знак</b> под сообщениями:\n・ Текущее: <code>{watermark_value}</code>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='other').pack()))
...
async def callback_enter_new_included_restore_item_keyphrases(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_restore_included_float_text(f'🔑 Введите <b>ключевые фразы</b> названия товара, который нужно включить в авто-восстановление (указываются через запятую, например, "samp аккаунт, со всеми данными"):'), reply_markup=templ.back_kb(calls.IncludedRestoreItemsPagination(page=last_page).pack()))
...
async def callback_enter_new_excluded_restore_item_keyphrases(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_restore_excluded_float_text(f'🔑 Введите <b>ключевые фразы</b> названия товара, который нужно исключить из авто-восстановления (указываются через запятую, например, "samp аккаунт, со всеми данными"):'), reply_markup=templ.back_kb(calls.ExcludedRestoreItemsPagination(page=last_page).pack()))
...
async def callback_enter_new_included_complete_deal_keyphrases(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_complete_included_float_text(f'🔑 Введите <b>ключевые фразы</b> названия товара, сделку по которому нужно включить в авто-подтверждение (указываются через запятую, например, "samp аккаунт, со всеми данными"):'), reply_markup=templ.back_kb(calls.IncludedCompleteDealsPagination(page=last_page).pack()))
...
async def callback_enter_new_excluded_complete_deal_keyphrases(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_complete_excluded_float_text(f'🔑 Введите <b>ключевые фразы</b> названия товара, сделку по которому нужно исключить из авто-подтверждения (указываются через запятую, например, "samp аккаунт, со всеми данными"):'), reply_markup=templ.back_kb(calls.ExcludedCompleteDealsPagination(page=last_page).pack()))
...
async def callback_enter_auto_bump_items_interval(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=callback.message, text=templ.settings_bump_float_text(f'⏲️ Введите <b>интервал поднятия предметов</b>:\n・ Текущее: <code>{interval}</code> сек.'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='bump').pack()))
    except:
        import traceback
        traceback.print_exc()
...
async def callback_enter_new_included_bump_item_keyphrases(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_bump_included_float_text(f'🔑 Введите <b>ключевые фразы</b> названия товара, который нужно включить в авто-поднятие (указываются через запятую, например, "samp аккаунт, со всеми данными"):'), reply_markup=templ.back_kb(calls.IncludedBumpItemsPagination(page=last_page).pack()))
...
async def callback_enter_new_excluded_bump_item_keyphrases(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_bump_excluded_float_text(f'🔑 Введите <b>ключевые фразы</b> названия товара, который нужно исключить из авто-поднятия (указываются через запятую, например, "samp аккаунт, со всеми данными"):'), reply_markup=templ.back_kb(calls.ExcludedBumpItemsPagination(page=last_page).pack()))
...
async def callback_enter_custom_commands_page(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_comms_float_text(f'📃 Enter the page number to go to:'), reply_markup=templ.back_kb(calls.CustomCommandsPagination(page=last_page).pack()))
...
async def callback_enter_new_custom_command(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_comm_float_text(f'⌨️ Введите <b>новую команду</b> (например, <code>!тест</code>):'), reply_markup=templ.back_kb(calls.CustomCommandsPagination(page=last_page).pack()))
...
async def callback_enter_custom_command_answer(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=callback.message, text=templ.settings_comm_page_float_text(f'💬 Введите новый <b>текст ответа</b> команды <code>{command}</code>:\n・ Текущее: <blockquote>{custom_command_answer}</blockquote>'), reply_markup=templ.back_kb(calls.CustomCommandPage(command=command).pack()))
...
async def callback_enter_auto_deliveries_page(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_delivs_float_text(f'📃 Enter the page number to go to:'), reply_markup=templ.back_kb(calls.AutoDeliveriesPagination(page=last_page).pack()))
...
async def callback_enter_new_auto_delivery_keyphrases(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_new_deliv_float_text(f'🔑 Введите <b>ключевые фразы</b> названия товара, на который нужно добавить авто-выдачу (указываются через запятую, например, "telegram подписчики, авто-выдача"):'), reply_markup=templ.back_kb(calls.AutoDeliveriesPagination(page=last_page).pack()))
...
async def callback_enter_auto_delivery_keyphrases(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=callback.message, text=templ.settings_deliv_page_float_text(f'🔑 Введите новые <b>ключевые фразы</b> названия товара, на который авто-выдачи (указываются через запятую)\n・ Текущее: <code>{auto_delivery_message}</code>'), reply_markup=templ.back_kb(calls.AutoDeliveryPage(index=index).pack()))
    except Exception as e:
...
async def callback_enter_auto_withdrawal_interval(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_withdrawal_float_text(f'⏱️ Введите новый <b>интервал вывода средств</b> (в секундах):\n・ Текущее: <code>{interval}</code> сек.'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='withdrawal').pack()))
...
async def callback_enter_usdt_address(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_withdrawal_usdt_float_text(f'💲 Введите <b>адрес кошелька</b> USDT (TRC20):'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='withdrawal').pack()))
...
async def callback_enter_messages_page(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_mess_float_text(f'📃 Enter the page number to go to:'), reply_markup=templ.back_kb(calls.MessagesPagination(page=last_page).pack()))
...
async def callback_enter_message_text(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=callback.message, text=templ.settings_mess_float_text(f'💬 Введите новый <b>текст сообщения</b> <code>{message_id}</code>:\n・ Текущее: <blockquote>{mess_text}</blockquote>'), reply_markup=templ.back_kb(calls.MessagesPagination(page=last_page).pack()))
    except Exception as e:
...
async def callback_enter_tg_logging_chat_id(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_logger_float_text(f'💬 Введите новый <b>ID чата для логов</b> (вы можете указать как цифровой ID, так и юзернейм чата):\n・ Текущее: <code>{tg_logging_chat_id}</code>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='logger').pack()))
