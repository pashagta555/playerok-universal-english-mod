...
async def callback_enter_auto_bump_items_interval(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_bump_float_text(f"⏲️ Enter <b>bump items interval</b> ↓"), 
...
async def callback_enter_new_included_bump_item_keyphrases(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_new_bump_included_float_text(f"🔑 Enter <b>key phrases</b> of item name to include in auto-bump (separated by commas, e.g., \"samp account, with all data\") ↓"), 
...
async def callback_enter_new_excluded_bump_item_keyphrases(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_new_bump_excluded_float_text(f"🔑 Enter <b>key phrases</b> of item name to exclude from auto-bump (separated by commas, e.g., \"samp account, with all data\") ↓"), 
...
async def callback_enter_custom_command_answer(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not custom_command:
            raise Exception("❌ Custom command was not found, restart the process from the beginning")
...
async def callback_enter_new_auto_delivery_keyphrases(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        state=state, 
        message=callback.message, 
        text=templ.settings_new_deliv_float_text(f"🔑 Enter <b>key phrases</b> of item name to add auto-delivery to (separated by commas, e.g., \"telegram subscribers, auto-delivery\") ↓"), 
        reply_markup=templ.back_kb(calls.AutoDeliveriesPagination(page=last_page).pack())
    )
...
async def callback_enter_message_text(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not message_id:
            raise Exception("❌ Message ID was not found, restart the process from the beginning")
