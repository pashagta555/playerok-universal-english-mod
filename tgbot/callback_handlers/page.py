...
async def callback_custom_command_page(callback: CallbackQuery, callback_data: calls.CustomCommandPage, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_comm_page_text(command), reply_markup=templ.settings_comm_page_kb(command, last_page), callback=callback)
...
async def callback_module_page(callback: CallbackQuery, callback_data: calls.ModulePage, state: FSMContext):
    ...
    data = await state.get_data()
    last_page = data.get('last_page', 0)
    await throw_float_message(state=state, message=callback.message, text=templ.module_page_text(module_uuid), reply_markup=templ.module_page_kb(module_uuid, last_page), callback=callback)