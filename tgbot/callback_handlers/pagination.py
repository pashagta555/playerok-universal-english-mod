...
async def callback_excluded_restore_items_pagination(callback: CallbackQuery, callback_data: calls.ExcludedRestoreItemsPagination, state: FSMContext):
    ...
    page = callback_data.page
    await state.update_data(last_page=page)
    await throw_float_message(state=state, message=callback.message, text=templ.settings_restore_excluded_text(), reply_markup=templ.settings_restore_excluded_kb(page), callback=callback)
...
async def callback_excluded_complete_deals_pagination(callback: CallbackQuery, callback_data: calls.ExcludedCompleteDealsPagination, state: FSMContext):
    ...
    page = callback_data.page
    await state.update_data(last_page=page)
    await throw_float_message(state=state, message=callback.message, text=templ.settings_complete_excluded_text(), reply_markup=templ.settings_complete_excluded_kb(page), callback=callback)
...
async def callback_modules_pagination(callback: CallbackQuery, callback_data: calls.ModulesPagination, state: FSMContext):
    ...
    page = callback_data.page
    await state.update_data(last_page=page)
    await throw_float_message(state=state, message=callback.message, text=templ.modules_text(), reply_markup=templ.modules_kb(page), callback=callback)
...
async def callback_bank_cards_pagination(callback: CallbackQuery, callback_data: calls.BankCardsPagination, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_withdrawal_cards_text(bank_cards), reply_markup=templ.settings_withdrawal_cards_kb(bank_cards, page), callback=callback)
...
async def callback_sbp_banks_pagination(callback: CallbackQuery, callback_data: calls.SbpBanksPagination, state: FSMContext):
    ...
    await throw_float_message(state=state, message=callback.message, text=templ.settings_withdrawal_sbp_text(sbp_banks), reply_markup=templ.settings_withdrawal_sbp_kb(sbp_banks, page), callback=callback)