...
async def callback_switch_message_enabled(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not message_id:
            raise Exception("❌ Message ID was not found, please restart the process from the beginning")
...
async def callback_switch_module_enabled(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not module:
            raise Exception("❌ Module with this UUID was not found, please restart the process from the beginning")
