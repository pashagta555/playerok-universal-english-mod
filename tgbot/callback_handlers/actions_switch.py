...
from ..callback_handlers.page import callback_message_page, callback_module_page, callback_auto_delivery_page
...
async def callback_switch_module_enabled(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if not all((module_uuid, module)):
            return await callback_modules_pagination(callback, calls.ModulesPagination(page=last_page), state)
        if module.enabled:
