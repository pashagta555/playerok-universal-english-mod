...
def instruction_comms_text():
    txt = textwrap.dedent(f"""
        ...
        ・ <code>!seller</code> — notifies and calls seller to dialog with buyer (sends you a Telegram message asking for help)
    """)
    return txt
...
def instruction_comms_kb():
    rows = [
        [InlineKeyboardButton(text="⬅️ Back", callback_data=calls.InstructionNavigation(to="default").pack())]
    ]
    kb = InlineKeyboardMarkup(inline_keyboard=rows)
    return kb