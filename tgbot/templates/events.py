...
def events_text():
    ...
    last_bump_items = (datetime.fromisoformat(latest_events_times["auto_bump_items"]).strftime("%d.%m.%Y %H:%M")) if latest_events_times.get("auto_bump_items") else "❌ There wasn't"
    ...
    last_withdrawal = (datetime.fromisoformat(latest_events_times["auto_withdrawal"]).strftime("%d.%m.%Y %H:%M")) if latest_events_times.get("auto_withdrawal") else "❌ There wasn't"
...
def events_kb():
    rows = [
        [InlineKeyboardButton(text="⬆️ Pick up objects", callback_data="confirm_bump_items")],
