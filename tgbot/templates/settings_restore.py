...
def settings_restore_text ():
    ...
    txt =textwrap .dedent (f"""<b>♻️ Auto-recovery</b>

        <b>♻️ Auto-recovery of items:</b>
        <b>・ Sold:</b>{auto_restore_items_sold }<b>・ Expired:</b>{auto_restore_items_expired }<b>📦 Restore:</b>{auto_restore_items_all }<b>➕ Included:</b>{auto_restore_items_included }<b>➖ Excluded:</b>{auto_restore_items_excluded }<b>What kind of auto-recovery of items?</b>
...
def settings_restore_float_text (placeholder :str ):
    txt =textwrap .dedent (f"""<b>♻️ Author-recovery</b>
        \n{placeholder }
    """)
    return txt 