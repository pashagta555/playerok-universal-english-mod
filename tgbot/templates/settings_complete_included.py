...
def settings_complete_included_kb(page=0):
    ...
    for keyphrases in list(included_complete_deals)[start_offset:end_offset]:
        ...
        rows.append([InlineKeyboardButton(text=f'{keyphrases_frmtd}', callback_data='123'), InlineKeyboardButton(text=f'🗑️', callback_data=calls.DeleteIncludedCompleteDeal(index=included_complete_deals.index(keyphrases)).pack())])
    if total_pages > 1:
        buttons_row = []
    ...
    rows.append([InlineKeyboardButton(text='➕ Add', callback_data='enter_new_included_complete_deal_keyphrases'), InlineKeyboardButton(text='➕📄 Add много', callback_data='send_new_included_complete_deals_keyphrases_file')])
    rows.append([InlineKeyboardButton(text='⬅️ Back', callback_data=calls.SettingsNavigation(to='complete').pack())])
