...
def settings_delivs_kb(page=0):
    ...
    for deliv in list(auto_deliveries)[start_offset:end_offset]:
        ...
        if piece:
            ...
            part = f'{len(goods)} items'
        else:
            message = deliv.get('message', [])
            part = '\n'.join(message) or '❌ Not specified'
        rows.append([InlineKeyboardButton(text=f'{sym} {keyphrases_frmtd} → {part}', callback_data=calls.AutoDeliveryPage(index=auto_deliveries.index(deliv)).pack())])
    if total_pages > 1:
        buttons_row = []
...
def settings_new_deliv_piece_kb(last_page=0):
    rows = [[InlineKeyboardButton(text='📦 Несколько items', callback_data=calls.SetNewDelivPiece(val=True).pack())], [InlineKeyboardButton(text='💬 One message', callback_data=calls.SetNewDelivPiece(val=False).pack())], [InlineKeyboardButton(text='⬅️ Back', callback_data=calls.AutoDeliveriesPagination(page=last_page).pack())]]
    kb = InlineKeyboardMarkup(inline_keyboard=rows)
    return kb