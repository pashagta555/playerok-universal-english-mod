...
Total <b>{len(auto_deliveries)}</b> products with automatic delivery:
    """)
    return txt
...
def settings_new_deliv_piece_kb(last_page=0):
    rows = [
[InlineKeyboardButton(text="📦 Multiple Products", callback_data=calls.SetNewDelivPiece(val=True).pack())],
