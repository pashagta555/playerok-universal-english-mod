...
def settings_withdrawal_cards_kb(bank_cards: list[UserBankCard], page=0):
    ...
    for card in list(bank_cards)[start_offset:end_offset]:
        ...
        rows.append([InlineKeyboardButton(text=f'{card_number} ({card.card_type.name})', callback_data=calls.SelectBankCard(id=card.id).pack())])
    if total_pages > 1:
        buttons_row = []
    ...
    rows.append([InlineKeyboardButton(text='· 💳 Maps RU ·', callback_data='123'), InlineKeyboardButton(text='📱 SBP banks', callback_data=calls.SbpBanksPagination(page=0).pack()), InlineKeyboardButton(text='💲 USDT (TRC20)', callback_data='enter_usdt_address')])
    rows.append([InlineKeyboardButton(text='⬅️ Back', callback_data=calls.SettingsNavigation(to='withdrawal').pack())])
