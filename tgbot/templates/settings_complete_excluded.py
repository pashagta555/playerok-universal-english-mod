...
def settings_complete_excluded_kb(page=0):
    ...
    for keyphrases in list(excluded_complete_deals)[start_offset:end_offset]:
        ...
        rows.append([InlineKeyboardButton(text=f'{keyphrases_frmtd}', callback_data='123'), InlineKeyboardButton(text=f'🗑️', callback_data=calls.DeleteExcludedCompleteDeal(index=excluded_complete_deals.index(keyphrases)).pack())])
    if total_pages > 1:
        buttons_row = []
    ...
    rows.append([InlineKeyboardButton(text='➕ Add', callback_data='enter_new_excluded_complete_deal_keyphrases'), InlineKeyboardButton(text='➕📄 Add много', callback_data='send_new_excluded_complete_deals_keyphrases_file')])
