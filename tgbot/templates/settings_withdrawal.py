...
def settings_withdrawal_kb(card: UserBankCard=None, sbp_bank: SBPBankMember=None):
    ...
    if card:
        ...
        details = 'Not specified'
    rows = [[InlineKeyboardButton(text=f'🔃 Авто-вывод средств: {enabled}', callback_data='switch_auto_withdrawal_enabled')], [InlineKeyboardButton(text=f'⏱️ Interval: {interval} sec.', callback_data='enter_auto_withdrawal_interval')], [InlineKeyboardButton(text=f'💳 Details: {details}', callback_data=calls.BankCardsPagination(page=0).pack())], [InlineKeyboardButton(text='⬅️ Back', callback_data=calls.SettingsNavigation(to='default').pack())]]
