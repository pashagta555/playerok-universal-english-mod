...
def settings_bump_text():
    ...
    txt = textwrap.dedent(f"""
        ...
        Бот будет автоматически поднимать предметы, которые выйдут за указанную позицию в таблице общих товаров. То есть, будет обновлять их PREMIUM Status of the, чтобы они снова были в топе. Позволяет обходить конкурентов, тем самым получая больше клиентов.
