...
def settings_other_text():
    ...
    txt = textwrap.dedent(f"""
        ...
        <b>👀 Mark chat as read before sending:</b> {switch_read_chat_enabled}
        <b>☑️ Auto-complete deals:</b> {auto_complete_deals_enabled}
        <b>❗ Commands:</b> {custom_commands_enabled}
        <b>🚀 Auto-delivery:</b> {auto_deliveries_enabled}
        
        <b>©️ Message watermark:</b> {watermark_enabled}
...
def settings_other_kb():
    ...
    watermark_value = config["playerok"]["watermark"]["value"] or "❌ Not set"
    rows = [
        [InlineKeyboardButton(text=f"👀 Mark chat as read before sending: {switch_read_chat_enabled}", callback_data="switch_read_chat_enabled")],
        [InlineKeyboardButton(text=f"☑️ Auto-complete deals: {auto_complete_deals_enabled}", callback_data="switch_auto_complete_deals_enabled")],
