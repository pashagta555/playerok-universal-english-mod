...
def settings_other_text():
    ...
    txt = textwrap.dedent(f"""
        ...
        <b>👀 Read chat before sending a message:</b> {switch_read_chat_enabled}
        <b>☑️ Auto-confirm deals:</b> {auto_complete_deals_enabled}
...
def settings_other_kb():
    ...
    watermark_value = config["playerok"]["watermark"]["value"] or "❌ Not set"
    rows = [
        [InlineKeyboardButton(text=f"👀 Read chat before sending a message: {switch_read_chat_enabled}", callback_data="switch_read_chat_enabled")],
        [InlineKeyboardButton(text=f"☑️ Auto-confirm deals: {auto_complete_deals_enabled}", callback_data="switch_auto_complete_deals_enabled")],
