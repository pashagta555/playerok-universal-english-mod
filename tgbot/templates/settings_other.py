...
def settings_other_text ():
    ...
    txt =textwrap .dedent (f"""<b>🔧 Other</b>

        <b>👀 Reading chat before sending a message:</b>{switch_read_chat_enabled }<b>❗ Commands:</b>{custom_commands_enabled }<b>🚀 Auto-issue:</b>{auto_deliveries_enabled }<b>©️ Watermark under messages:</b>{watermark_enabled }<b>✍️©️ Watermark:</b>{watermark_value }
    """)
    return txt 
...
def settings_other_kb ():
    ...
    rows =[
    [InlineKeyboardButton (text =f"👀 Reading the chat before sending a message:{switch_read_chat_enabled }",callback_data ="switch_read_chat_enabled")],
    [InlineKeyboardButton (text =f"❗ Teams:{custom_commands_enabled }",callback_data ="switch_custom_commands_enabled")],
    [InlineKeyboardButton (text =f"🚀 Auto-issue:{auto_deliveries_enabled }",callback_data ="switch_auto_deliveries_enabled")],
