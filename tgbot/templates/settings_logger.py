...
def settings_logger_kb():
    ...
    event_new_problem = '🟢' if tg_logging_events['new_problem'] else '🔴'
    event_deal_status_changed = '🟢' if tg_logging_events['deal_status_changed'] else '🔴'
    rows = [[InlineKeyboardButton(text=f'👀 Event logging: {tg_logging_enabled}', callback_data='switch_tg_logging_enabled')], [InlineKeyboardButton(text=f'💬 Chat ID для логов: {tg_logging_chat_id}', callback_data='enter_tg_logging_chat_id')], [InlineKeyboardButton(text=f'{event_new_user_message}  👤 New user message', callback_data='switch_tg_logging_event_new_user_message'), InlineKeyboardButton(text=f'{event_new_system_message}  ⚙️ New system message', callback_data='switch_tg_logging_event_new_system_message')], [InlineKeyboardButton(text=f'{event_new_deal}  📋 New deal', callback_data='switch_tg_logging_event_new_deal'), InlineKeyboardButton(text=f'{event_new_review}  ✨ New review', callback_data='switch_tg_logging_event_new_review')], [InlineKeyboardButton(text=f'{event_new_problem}  🤬 New complaint in the transaction', callback_data='switch_tg_logging_event_new_problem'), InlineKeyboardButton(text=f'{event_deal_status_changed}  🔄️ Deal status changed', callback_data='switch_tg_logging_event_deal_status_changed')], [InlineKeyboardButton(text='⬅️ Back', callback_data=calls.SettingsNavigation(to='default').pack())]]
