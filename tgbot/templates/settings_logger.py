...
def settings_logger_text():
    ...
    txt = textwrap.dedent(f"""
        ...
        <b>👀 Logging events:</b> {tg_logging_enabled}
        <b>💬 ID chat For lairs:</b> {tg_logging_chat_id}
        
        <b>📢 Events:</b>
        ・ {event_new_user_message}  👤 New message from user
        ・ {event_new_system_message}  ⚙️ New systemic message
        ・ {event_new_deal}  📋 New deal
        ・ {event_new_review}  ✨ New review
        ・ {event_new_problem}  🤬 New complaint V deal
        ・ {event_deal_status_changed}  🔄️ Status deals changed
    """)
    return txt
...
def settings_logger_kb():
    ...
    rows = [
        [InlineKeyboardButton(text=f"👀 Logging events: {tg_logging_enabled}", callback_data="switch_tg_logging_enabled")],
        [InlineKeyboardButton(text=f"💬 ID chat For lairs: {tg_logging_chat_id}", callback_data="enter_tg_logging_chat_id")],
        [
        ...
        InlineKeyboardButton(text=f"{event_new_system_message}  ⚙️ New systemic message", callback_data="switch_tg_logging_event_new_system_message"),
        ],
        [
        ...
        [
        InlineKeyboardButton(text=f"{event_new_problem}  🤬 New complaint V deal", callback_data="switch_tg_logging_event_new_problem"),
        InlineKeyboardButton(text=f"{event_deal_status_changed}  🔄️ Status deals changed", callback_data="switch_tg_logging_event_deal_status_changed")
        ],
