...
def settings_logger_text():
    ...
    txt = textwrap.dedent(f"""
        ...
        <b>💬 Chat ID for logs:</b> {tg_logging_chat_id}
        
        <b>📢 Events:</b>
        <b>・ {event_new_user_message}  💬👤 New message from user</b>
        <b>・ {event_new_system_message}  💬⚙️ New system message</b>
        <b>・ {event_new_deal}  📋 New deal</b>
        <b>・ {event_new_review}  💬✨ New review</b>
        <b>・ {event_new_problem}  🤬 New complaint in deal</b>
        <b>・ {event_deal_status_changed}  🔄️📋 Deal status changed</b>
...
def settings_logger_kb():
    ...
    event_deal_status_changed = "🟢" if tg_logging_events["deal_status_changed"] else "🔴"
    rows = [
        [InlineKeyboardButton(text=f"👀 Playerok event logging in Telegram: {tg_logging_enabled}", callback_data="switch_tg_logging_enabled")],
