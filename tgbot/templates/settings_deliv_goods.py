...
def settings_deliv_goods_kb(index=0, page=0):
    ...
    for good in list(goods)[start_offset:end_offset]:
        rows.append([InlineKeyboardButton(text=str(good), callback_data='123'), InlineKeyboardButton(text='🗑️', callback_data=calls.DeleteDelivGood(index=goods.index(good)).pack())])
    if total_pages > 1:
        buttons_row = []
