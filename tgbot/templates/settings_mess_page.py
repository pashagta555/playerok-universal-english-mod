...
def settings_mess_page_text (message_id :int ):
    ...
    txt =textwrap .dedent (f"""<b>📄💬 Message page</b>

        <b>🆔 Message ID:</b>{message_id }<b>💡 Status:</b>{enabled }<b>💬 Message text:</b> <blockquote>{message_text }</blockquote>
    """)
    return txt 
...
def settings_mess_page_kb (message_id :int ,page :int =0 ):
    ...
    rows =[
    [InlineKeyboardButton (text =f"💡 Condition:{enabled }",callback_data ="switch_message_enabled")],
