...
def settings_withdrawal_sbp_kb(sbp_banks: list[SBPBankMember], page=0):
    ...
    for bank in list(sbp_banks)[start_offset:end_offset]:
        dynamic_btns.append(InlineKeyboardButton(text=bank.name, callback_data=calls.SelectSbpBank(id=bank.id).pack()))
    ...
    rows.append([InlineKeyboardButton(text='💳 Maps RU', callback_data=calls.BankCardsPagination(page=0).pack()), InlineKeyboardButton(text='· 📱 SBP banks ·', callback_data='123'), InlineKeyboardButton(text='💲 USDT (TRC20)', callback_data='enter_usdt_address')])
