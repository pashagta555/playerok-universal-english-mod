...
def settings_restore_excluded_text ():
    ...
    txt =textwrap .dedent (f"""<b>♻️➖ Excluded</b>

        Total<b>{len (excluded_restore_items )}</b>excluded items:""")
    return txt 
...
def settings_restore_excluded_kb (page =0 ):
    ...
    InlineKeyboardButton (text ='➕📄Add a lot',callback_data ="send_new_excluded_restore_items_keyphrases_file"),
    ])
    rows .append ([
...
def settings_new_restore_excluded_float_text (placeholder :str ):
    txt =textwrap .dedent (f"""<b>♻️➖ Adding an excluded item</b>
        \n{placeholder }
    """)
    return txt 