...
def settings_restore_excluded_kb(page=0):
    ...
    for keyphrases in list(excluded_restore_items)[start_offset:end_offset]:
        ...
        rows.append([InlineKeyboardButton(text=f'{keyphrases_frmtd}', callback_data='123'), InlineKeyboardButton(text=f'🗑️', callback_data=calls.DeleteExcludedRestoreItem(index=excluded_restore_items.index(keyphrases)).pack())])
    if total_pages > 1:
        buttons_row = []
    ...
    rows.append([InlineKeyboardButton(text='➕ Add', callback_data='enter_new_excluded_restore_item_keyphrases'), InlineKeyboardButton(text='➕📄 Add много', callback_data='send_new_excluded_restore_items_keyphrases_file')])
    rows.append([InlineKeyboardButton(text='⬅️ Back', callback_data=calls.SettingsNavigation(to='restore').pack())])
