...
def settings_complete_text ():
    ...
    txt =textwrap .dedent (f"""<b>☑️ Auto-confirmation</b>

        <b>☑️ Auto-confirmation of transactions:</b>{enabled }<b>📦 Confirm transactions:</b>{all }<b>➕ Included:</b>{included }<b>➖ Excluded:</b>{excluded }<b>What kind of auto-confirmation of transactions?</b>
...
def settings_complete_kb ():
    ...
    rows =[
    [InlineKeyboardButton (text =f"☑️ Auto-confirmation of transactions:{enabled }",callback_data ="switch_auto_complete_deals_enabled")],
    [InlineKeyboardButton (text =f"📦 Confirm transactions:{all }",callback_data ="switch_auto_complete_deals_all")],
    [
