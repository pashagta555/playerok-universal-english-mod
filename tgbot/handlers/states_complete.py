...
async def handler_waiting_for_new_included_complete_deal_keyphrases(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=message, text=templ.settings_new_complete_included_float_text('✅ Item successfully included in confirmation'), reply_markup=templ.back_kb(calls.IncludedCompleteDealsPagination(page=last_page).pack()))
...
async def handler_waiting_for_new_included_complete_deals_keyphrases_file(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('auto_complete_deals', auto_complete_deals)
        await throw_float_message(state=state, message=message, text=templ.settings_new_complete_included_float_text(f'✅ Successfully enabled <b>{len(keyphrases_list)} предметов</b> из файла в подтверждение'), reply_markup=templ.back_kb(calls.IncludedCompleteDealsPagination(page=last_page).pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_new_complete_included_float_text(e), reply_markup=templ.back_kb(calls.IncludedCompleteDealsPagination(page=last_page).pack()))
...
async def handler_waiting_for_new_excluded_complete_deal_keyphrases(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('auto_complete_deals', auto_complete_deals)
        await throw_float_message(state=state, message=message, text=templ.settings_new_complete_excluded_float_text('✅ Item successfully added to exceptions for confirmation'), reply_markup=templ.back_kb(calls.ExcludedCompleteDealsPagination(page=last_page).pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_new_complete_excluded_float_text(e), reply_markup=templ.back_kb(calls.ExcludedCompleteDealsPagination(page=last_page).pack()))

@router.message(states.CompleteDealsStates.waiting_for_new_excluded_complete_deals_keyphrases_file, F.document.file_name.lower().endswith('.txt'))
async def handler_waiting_for_new_excluded_complete_deals_keyphrases_file(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('auto_complete_deals', auto_complete_deals)
        await throw_float_message(state=state, message=message, text=templ.settings_new_complete_excluded_float_text(f'✅ Successfully added <b>{len(keyphrases_list)} предметов</b> из файла в исключения для подтверждения'), reply_markup=templ.back_kb(calls.ExcludedCompleteDealsPagination(page=last_page).pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_new_complete_excluded_float_text(e), reply_markup=templ.back_kb(calls.ExcludedCompleteDealsPagination(page=last_page).pack()))