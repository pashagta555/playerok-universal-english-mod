    ...
    try:
        ...
        if message.photo:
            ...
            if message.caption:
                ...
                await asyncio.sleep(1)
            plbot.account.send_message(chat_id=chat.id, photo_file_paths=photo_paths)
        ...
        plbot.account.send_message(chat_id=chat.id, text=text, photo_file_paths=photo_paths)
        await throw_float_message(state=state, message=message, text=templ.do_action_text(f'✅ To the user <b>{username}</b> было отправлено сообщение: <blockquote>{sent_msg.strip()}</blockquote>'), reply_markup=templ.destroy_kb())
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.do_action_text(e), reply_markup=templ.destroy_kb())