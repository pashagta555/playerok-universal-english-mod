...
async def handler_waiting_for_auto_deliveries_page(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=message, text=templ.settings_delivs_float_text(f'📃 Enter the page number to go to:'), reply_markup=templ.settings_delivs_kb(page))
...
async def handler_waiting_for_new_auto_delivery_keyphrases(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=message, text=templ.settings_new_deliv_float_text(f'🛒 Выберите <b>тип авто-выдачи</b>:'), reply_markup=templ.settings_new_deliv_piece_kb(last_page))
    except Exception as e:
...
async def handler_waiting_for_new_auto_delivery_goods(message: types.Message, state: FSMContext):
    try:
        ...
        if not goods:
            raise Exception('❌ Failed to retrieve items')
        await state.update_data(new_auto_delivery_goods=goods)
        keyphrases = data.get('new_auto_delivery_keyphrases')
        phrases = '</code>, <code>'.join(keyphrases)
        await throw_float_message(state=state, message=message, text=templ.settings_new_deliv_float_text(f'✔️ Подтвердите <b>добавление авто-выдачи</b>:\n<b>· Ключевые фразы:</b> <code>{phrases}</code>\n<b>· Тип выдачи:</b> Поштучно\n<b>· Товары:</b> {len(goods)} pcs.'), reply_markup=templ.confirm_kb(confirm_cb='add_new_auto_delivery', cancel_cb=calls.AutoDeliveriesPagination(page=last_page).pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_new_deliv_float_text(e), reply_markup=templ.back_kb(calls.AutoDeliveriesPagination(page=last_page).pack()))
...
async def handler_waiting_for_auto_delivery_message(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('auto_deliveries', auto_deliveries)
        await throw_float_message(state=state, message=message, text=templ.settings_deliv_page_float_text(f'✅ <b>Сообщение авто-выдачи</b> было успешно изменено на: <blockquote>{message.text}</blockquote>'), reply_markup=templ.back_kb(calls.AutoDeliveryPage(index=index).pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_deliv_page_float_text(e), reply_markup=templ.back_kb(calls.AutoDeliveryPage(index=index).pack()))
...
async def handler_waiting_for_auto_delivery_goods_add(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('auto_deliveries', auto_deliveries)
        await throw_float_message(state=state, message=message, text=templ.settings_new_deliv_goods_float_text(f'✅ <b>{len(goods)} товаров</b> успешно добавлено в авто-выдачу'), reply_markup=templ.back_kb(calls.DelivGoodsPagination(page=last_page).pack()))
    except Exception as e:
