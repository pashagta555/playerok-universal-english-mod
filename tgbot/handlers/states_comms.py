...
async def handler_waiting_for_new_custom_command(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=message,
            text=templ.settings_new_comm_float_text(f"💬 Enter <b>reply for command</b> <code>{message.text.strip()}</code> ↓"),
            reply_markup=templ.back_kb(calls.CustomCommandsPagination(page=data.get("last_page", 0)).pack())
        )
    except Exception as e:
        data = await state.get_data()
...
async def handler_waiting_for_custom_command_answer(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=message,
            text=templ.settings_comm_page_float_text(f"✅ <b>Reply text</b> for command <code>{data['custom_command']}</code> was successfully changed to: <blockquote>{message.text.strip()}</blockquote>"),
