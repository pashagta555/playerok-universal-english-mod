...
async def handler_waiting_for_new_custom_command(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=message, text=templ.settings_new_comm_float_text(f'💬 Введите <b>ответ для команды</b> <code>{message.text.strip()}</code>:'), reply_markup=templ.back_kb(calls.CustomCommandsPagination(page=data.get('last_page', 0)).pack()))
    except Exception as e:
        data = await state.get_data()
        await throw_float_message(state=state, message=message, text=templ.settings_new_comm_float_text(e), reply_markup=templ.back_kb(calls.CustomCommandsPagination(page=data.get('last_page', 0)).pack()))
...
async def handler_waiting_for_new_custom_command_answer(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=message, text=templ.settings_new_comm_float_text(f'✔️ Подтвердите <b>добавление новой команды:</b>\n<b>· Команда:</b> {cmd}\n<b>· Ответ:</b> <blockquote>{answr}</blockquote>'), reply_markup=templ.confirm_kb(confirm_cb='add_new_custom_command', cancel_cb=calls.CustomCommandsPagination(page=data.get('last_page', 0)).pack()))
    except Exception as e:
        data = await state.get_data()
        await throw_float_message(state=state, message=message, text=templ.settings_new_comm_float_text(e), reply_markup=templ.back_kb(calls.CustomCommandsPagination(page=data.get('last_page', 0)).pack()))
...
async def handler_waiting_for_custom_command_answer(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=message, text=templ.settings_comm_page_float_text(f"✅ <b>Текст ответа</b> команды <code>{data['custom_command']}</code> был успешно изменён на: <blockquote>{message.text.strip()}</blockquote>"), reply_markup=templ.back_kb(calls.CustomCommandPage(command=data['custom_command']).pack()))
    except Exception as e:
        data = await state.get_data()
        await throw_float_message(state=state, message=message, text=templ.settings_comm_page_float_text(e), reply_markup=templ.back_kb(calls.CustomCommandPage(command=data['custom_command']).pack()))