...
async def handler_waiting_for_bump_items_interval(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=message, text=templ.settings_bump_float_text(f'✅ <b>Интервал поднятия предметов</b> был успешно изменён на <b>{interval}</b>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='bump').pack()))
...
async def handler_waiting_for_new_included_bump_item_keyphrases(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=message, text=templ.settings_new_bump_included_float_text(f"✅ Item with keywords <code>{'</code>, <code>'.join(keyphrases)}</code> успешно включён в поднятие"), reply_markup=templ.back_kb(calls.IncludedBumpItemsPagination(page=last_page).pack()))
    except Exception as e:
        data = await state.get_data()
        last_page = data.get('last_page', 0)
...
async def handler_waiting_for_new_included_bump_items_keyphrases_file(message: types.Message, state: FSMContext):
    try:
        ...
        data = await state.get_data()
        last_page = data.get('last_page', 0)
        await throw_float_message(state=state, message=message, text=templ.settings_new_bump_included_float_text(f'✅ Successfully enabled <b>{len(keyphrases_list)}</b> предметов из файла в поднятие'), reply_markup=templ.back_kb(calls.IncludedBumpItemsPagination(page=last_page).pack()))
    except Exception as e:
        data = await state.get_data()
        last_page = data.get('last_page', 0)
        await throw_float_message(state=state, message=message, text=templ.settings_new_bump_included_float_text(e), reply_markup=templ.back_kb(calls.IncludedBumpItemsPagination(page=last_page).pack()))
...
async def handler_waiting_for_new_excluded_bump_item_keyphrases(message: types.Message, state: FSMContext):
    try:
        ...
        data = await state.get_data()
        last_page = data.get('last_page', 0)
        await throw_float_message(state=state, message=message, text=templ.settings_new_bump_excluded_float_text(f"✅ Item with keywords <code>{'</code>, <code>'.join(keyphrases)}</code> успешно добавлен в исключения для поднятия"), reply_markup=templ.back_kb(calls.ExcludedBumpItemsPagination(page=last_page).pack()))
    except Exception as e:
        data = await state.get_data()
        last_page = data.get('last_page', 0)
...
async def handler_waiting_for_new_excluded_bump_items_keyphrases_file(message: types.Message, state: FSMContext):
    try:
        ...
        data = await state.get_data()
        last_page = data.get('last_page', 0)
        await throw_float_message(state=state, message=message, text=templ.settings_new_bump_excluded_float_text(f'✅ Successfully added <b>{len(keyphrases_list)}</b> предметов из файла в исключения для поднятия'), reply_markup=templ.back_kb(calls.ExcludedBumpItemsPagination(page=last_page).pack()))
    except Exception as e:
        data = await state.get_data()
        last_page = data.get('last_page', 0)
        await throw_float_message(state=state, message=message, text=templ.settings_new_bump_excluded_float_text(e), reply_markup=templ.back_kb(calls.ExcludedBumpItemsPagination(page=last_page).pack()))