...
async def handler_waiting_for_new_included_bump_item_keyphrases(message: types.Message, state: FSMContext):
    try: 
        ...
        await throw_float_message(
            state=state,
            message=message,
            text=templ.settings_new_bump_included_float_text(f"✅ Item with key phrases <code>{'</code>, <code>'.join(keyphrases)}</code> successfully included in raising"),
...
async def handler_waiting_for_new_included_bump_items_keyphrases_file(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=message,
            text=templ.settings_new_bump_included_float_text(f"✅ Successfully enabled <b>{len(keyphrases_list)}</b> items from the file to pick up"),
            reply_markup=templ.back_kb(calls.IncludedBumpItemsPagination(page=last_page).pack())
        )
    except Exception as e:
        data = await state.get_data()
