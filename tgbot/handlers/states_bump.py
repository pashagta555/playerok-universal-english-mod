...
async def handler_waiting_for_bump_items_interval (message :types .Message ,state :FSMContext ):
    try :
        ...
        state =state ,
        message =message ,
        text =templ .settings_bump_float_text (f"✅ <b>Item pickup interval</b> has been successfully changed to <b>{interval }</b>"),
...
async def handler_waiting_for_new_included_bump_item_keyphrases (message :types .Message ,state :FSMContext ):
    try :
        ...
        state =state ,
        message =message ,
        text =templ .settings_new_bump_included_float_text (f"✅ Item with keywords <code>{'</code>, <code>'.join (keyphrases )}</code> successfully included in raising"),
...
async def handler_waiting_for_new_included_bump_items_keyphrases_file (message :types .Message ,state :FSMContext ):
    try :
        ...
        state =state ,
        message =message ,
        text =templ .settings_new_bump_included_float_text (f"✅ Successfully enabled <b>{len (keyphrases_list )}</b> items from file to pick up"),
        reply_markup =templ .back_kb (calls .IncludedBumpItemsPagination (page =last_page ).pack ())
        )
    except Exception as e :
        data =await state .get_data ()
...
async def handler_waiting_for_new_excluded_bump_item_keyphrases (message :types .Message ,state :FSMContext ):
    try :
        ...
        state =state ,
        message =message ,
        text =templ .settings_new_bump_excluded_float_text (f"✅ Item with keywords <code>{'</code>, <code>'.join (keyphrases )}</code> successfully added to exceptions for raising"),
...
async def handler_waiting_for_new_excluded_bump_items_keyphrases_file (message :types .Message ,state :FSMContext ):
    try :
        ...
        state =state ,
        message =message ,
        text =templ .settings_new_bump_excluded_float_text (f"✅ Successfully added <b>{len (keyphrases_list )}</b> items from the file into exceptions for raising"),
        reply_markup =templ .back_kb (calls .ExcludedBumpItemsPagination (page =last_page ).pack ())
        )
    except Exception as e :
        data =await state .get_data ()
