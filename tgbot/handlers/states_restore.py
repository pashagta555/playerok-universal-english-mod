...
async def handler_waiting_for_new_included_restore_item_keyphrases(message: types.Message, state: FSMContext):
    try: 
        ...
        await throw_float_message(
            state=state,
            message=message,
            text=templ.settings_new_restore_included_float_text(f"✅ Item with keyphrases <code>{'</code>, <code>'.join(keyphrases)}</code> was successfully included for restore"),
...
async def handler_waiting_for_new_included_restore_items_keyphrases_file(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=message,
            text=templ.settings_new_restore_included_float_text(f"✅ Successfully included <b>{len(keyphrases_list)}</b> items from the file for restore"),
            reply_markup=templ.back_kb(calls.IncludedRestoreItemsPagination(page=last_page).pack())
        )
    except Exception as e:
        data = await state.get_data()
...
async def handler_waiting_for_new_excluded_restore_item_keyphrases(message: types.Message, state: FSMContext):
    try: 
        ...
        await throw_float_message(
            state=state,
            message=message,
            text=templ.settings_new_restore_excluded_float_text(f"✅ Item with keyphrases <code>{'</code>, <code>'.join(keyphrases)}</code> was successfully added to restore exclusions"),
...
async def handler_waiting_for_new_excluded_restore_items_keyphrases_file(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=message,
            text=templ.settings_new_restore_excluded_float_text(f"✅ Successfully added <b>{len(keyphrases_list)}</b> items from the file to restore exclusions"),
            reply_markup=templ.back_kb(calls.ExcludedRestoreItemsPagination(page=last_page).pack())
        )
    except Exception as e:
        data = await state.get_data()
