...
from utils import is_cookies_valid, is_token_valid, is_user_agent_valid, is_proxy_valid, is_proxy_working
router = Router()
...
async def handler_waiting_for_cookies(message: types.Message, state: FSMContext):
    try:
        ...
        if not is_cookies_valid(str_cookies) or not is_token_valid(cookies['token']):
            raise Exception('❌ Неверные Cookies. Убедитесь, что они указаны в формате Header String')
        config = sett.get('config')
        config['playerok']['api']['cookies'] = str_cookies
        sett.set('config', config)
        await throw_float_message(state=state, message=message, text=templ.settings_auth_float_text(f'🍪 <b>Cookie-данные</b> были успешно изменены на: <blockquote>{str_cookies}</blockquote>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='auth').pack()))
...
async def handler_waiting_for_pl_proxy(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('config', config)
        await throw_float_message(state=state, message=message, text=templ.settings_auth_float_text(f'✅ <b>Прокси для Playerok</b> был успешно изменён на <b>{proxy}</b>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='conn').pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_auth_float_text(e), reply_markup=templ.back_kb(calls.SettingsNavigation(to='conn').pack()))
...
async def handler_waiting_for_tg_proxy(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('config', config)
        await throw_float_message(state=state, message=message, text=templ.settings_auth_float_text(f'✅ <b>Прокси для Telegram</b> был успешно изменён на <b>{proxy}</b>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='conn').pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_auth_float_text(e), reply_markup=templ.back_kb(calls.SettingsNavigation(to='conn').pack()))
...
async def handler_waiting_for_requests_timeout(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('config', config)
        await throw_float_message(state=state, message=message, text=templ.settings_conn_float_text(f'✅ <b>Таймаут запросов</b> был успешно изменён на <b>{timeout}</b>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='conn').pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_conn_float_text(e), reply_markup=templ.back_kb(calls.SettingsNavigation(to='conn').pack()))
...
async def handler_waiting_for_listener_requests_delay(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('config', config)
        await throw_float_message(state=state, message=message, text=templ.settings_conn_float_text(f'✅ <b>Периодичность запросов</b> была успешна изменена на <b>{delay}</b>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='conn').pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_conn_float_text(e), reply_markup=templ.back_kb(calls.SettingsNavigation(to='conn').pack()))
...
async def handler_waiting_for_tg_logging_chat_id(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('config', config)
        await throw_float_message(state=state, message=message, text=templ.settings_logger_float_text(f'✅ <b>ID чата для логов</b> было успешно изменено на <b>{chat_id}</b>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='logger').pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_logger_float_text(e), reply_markup=templ.back_kb(calls.SettingsNavigation(to='logger').pack()))
...
async def handler_waiting_for_auto_withdrawal_interval(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('config', config)
        await throw_float_message(state=state, message=message, text=templ.settings_withdrawal_float_text(f'✅ <b>Интервал вывода</b> был успешно изменён на <b>{interval_int}</b>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='withdrawal').pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_withdrawal_float_text(e), reply_markup=templ.back_kb(calls.SettingsNavigation(to='withdrawal').pack()))
...
async def handler_waiting_for_sbp_bank_phone_number(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('config', config)
        await throw_float_message(state=state, message=message, text=templ.settings_withdrawal_sbp_float_text(f'✅ <b>Данные вывода</b> были успешно изменены на <b>{phone_number} (СБП)</b>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='withdrawal').pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_withdrawal_sbp_float_text(e), reply_markup=templ.back_kb(calls.SettingsNavigation(to='withdrawal').pack()))
...
async def handler_waiting_for_usdt_address(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('config', config)
        await throw_float_message(state=state, message=message, text=templ.settings_withdrawal_float_text(f'✅ <b>Данные вывода</b> были успешно изменены на <b>{address} (USDT TRC20)</b>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='withdrawal').pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_withdrawal_float_text(e), reply_markup=templ.back_kb(calls.SettingsNavigation(to='withdrawal').pack()))
...
async def handler_waiting_for_watermark_value(message: types.Message, state: FSMContext):
    try:
        ...
        sett.set('config', config)
        await throw_float_message(state=state, message=message, text=templ.settings_other_float_text(f'✅ <b>Водяной знак сообщений</b> был успешно изменён на <b>{watermark}</b>'), reply_markup=templ.back_kb(calls.SettingsNavigation(to='other').pack()))
    except Exception as e:
        await throw_float_message(state=state, message=message, text=templ.settings_other_float_text(e), reply_markup=templ.back_kb(calls.SettingsNavigation(to='other').pack()))
