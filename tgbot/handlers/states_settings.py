...
async def handler_waiting_for_sbp_bank_phone_number(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=message,
            text=templ.settings_withdrawal_sbp_float_text(f"✅ <b>Withdrawal details</b> were successfully changed to <b>{phone_number} (SBP)</b>"),
            reply_markup=templ.back_kb(calls.SettingsNavigation(to="withdrawal").pack())
        )
    except Exception as e:
        await throw_float_message(
...
async def handler_waiting_for_usdt_address(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=message,
            text=templ.settings_withdrawal_float_text(f"✅ <b>Withdrawal details</b> were successfully changed to <b>{address} (USDT TRC20)</b>"),
            reply_markup=templ.back_kb(calls.SettingsNavigation(to="withdrawal").pack())
        )
    except Exception as e:
        await throw_float_message(
...
async def handler_waiting_for_logs_max_file_size(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(
            state=state,
            message=message,
            text=templ.logs_float_text(f"✅ <b>Max log file size</b> was successfully changed to <b>{max_size} MB</b>"),
