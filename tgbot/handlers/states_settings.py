...
raise Exception("❌ Invalid token format. Example: eyJhbGciOiJIUzI1NiIsInR5cCI1IkpXVCJ9")
        ...
        await throw_float_message(
            state=state,
            message=message,
text=templ.settings_auth_float_text(f"✅ <b>Token</b> was successfully changed to <b>{token}</b>"),
...
raise Exception("❌ Invalid User Agent format. Example: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/144.0.0.0 Safari/537.36")
        ...
        await throw_float_message(
            state=state,
            message=message,
text=templ.settings_auth_float_text(f"✅ <b>User Agent</b> was successfully changed to <b>{user_agent}</b>"),
            reply_markup=templ.back_kb(calls.SettingsNavigation(to="auth").pack())
        )
    except Exception as e:
        await throw_float_message(
...
raise Exception("❌ Invalid proxy format. Correct format: user:pass@ip:port or ip:port")
        if not is_proxy_working(proxy):
raise Exception("❌ The proxy you specified is not working. There is no connection to playerok.com")
        ...
        await throw_float_message(
            state=state,
            message=message,
text=templ.settings_auth_float_text(f"✅ <b>Proxy for Playerok</b> was successfully changed to <b>{proxy}</b>"),
...
raise Exception("❌ The proxy you specified is not working. There is no connection to api.telegram.org")
        ...
        await throw_float_message(
            state=state,
            message=message,
text=templ.settings_auth_float_text(f"✅ <b>Proxy for Telegram</b> was successfully changed to <b>{proxy}</b>"),
            reply_markup=templ.back_kb(calls.SettingsNavigation(to="conn").pack())
        )
    except Exception as e:
        await throw_float_message(
...
raise Exception("❌ Value too low")
        ...
        await throw_float_message(
            state=state,
            message=message,
text=templ.settings_conn_float_text(f"✅ <b>Request timeout</b> was successfully changed to <b>{timeout}</b>"),
            reply_markup=templ.back_kb(calls.SettingsNavigation(to="conn").pack())
        )
    except Exception as e:
        await throw_float_message(
...
raise Exception("❌ Value too low")
        ...
        await throw_float_message(
            state=state,
            message=message,
text=templ.settings_conn_float_text(f"✅ <b>Request frequency</b> was successfully changed to <b>{delay}</b>"),
            reply_markup=templ.back_kb(calls.SettingsNavigation(to="conn").pack())
        )
    except Exception as e:
        await throw_float_message(
...
raise Exception("❌ Value too low")
        ...
        await throw_float_message(
            state=state,
            message=message,
text=templ.settings_withdrawal_float_text(f"✅ <b>Output interval</b> was successfully changed to <b>{interval_int}</b>"),
...
async def handler_waiting_for_sbp_bank_phone_number(message: types.Message, state: FSMContext):
    try:
        ...
        phone_number = message.textif not phone_number.isdigit():
        ...
        config = sett.get("config")
        config["player"]["auto_withdrawal"]["credentials_type"] = "sbp"
        ...
        await throw_float_message(
            state=state,
            message=message,
            text=templ.settings_withdrawal_sbp_float_text(f"✅ <b>Output data</b> was successfully changed to <b>{phone_number} (SBP)</b>"),
            reply_markup=templ.back_kb(calls.SettingsNavigation(to="withdrawal").pack())
        )
    except Exception as e:
        await throw_float_message(
...
async def handler_waiting_for_watermark_value(message: types.Message, state: FSMContext):
    try:
        ...
        if len(watermark) <= 0 or len(watermark) >= 150:
            raise Exception("❌ Value is too short or long")

        config = sett.get("config")
        config["player"]["watermark"]["value"] = watermark
        sett.set("config", config)
        
        await throw_float_message(
            state=state,
            message=message,
            text=templ.settings_other_float_text(f"✅ <b>Message watermark</b> was successfully changed to <b>{watermark}</b>"),
...
raise Exception("❌ Value too low")
        ...
        await throw_float_message(
            state=state,
            message=message,
text=templ.logs_float_text(f"✅ <b>Maximum log file size</b> was successfully changed to <b>{max_size_int} MB</b>"),
