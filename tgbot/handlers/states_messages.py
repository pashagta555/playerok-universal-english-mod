...
async def handler_waiting_for_message_text(message: types.Message, state: FSMContext):
    try:
        ...
        await throw_float_message(state=state, message=message, text=templ.settings_mess_page_float_text(f'✅ <b>Текст сообщения</b> <code>{message_id}</code> был успешно изменён на <blockquote>{message.text}</blockquote>'), reply_markup=templ.back_kb(calls.MessagePage(message_id=message_id).pack()))
    except Exception as e:
