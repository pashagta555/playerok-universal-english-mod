...
async def do_auth(message: Message, state: FSMContext) -> Message | None:
    ...
    return await throw_float_message(
        ...
        text=templ.sign_text(
            '🔑 Enter the key password you specified in the bot config ↓'
            '\n\n<span class="tg-spoiler">If you forgot it, you can see it directly in the config at bot_settings/config.json, the password parameter in the telegram.bot section</span>'
        ),
