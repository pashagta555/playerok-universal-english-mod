...
[![stars](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.github.com%2Frepos%2Fpashagta555%2Fplayerok-universal-english-mod&query=%24.stargazers_count&style=for-the-badge&label=stars&color=43d433&logo=github)](https://github.com/pashagta555/playerok-universal-english-mod/stargazers)
[![forks](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.github.com%2Frepos%2Fpashagta555%2Fplayerok-universal-english-mod&query=%24.forks_count&style=for-the-badge&label=forks&color=%236c70e6&logo=github)](https://github.com/pashagta555/playerok-universal-english-mod/forks)
...
### 🌐 More advanced
- Connect to HTTPS IPv4 proxy
- Setting request intervals

## ⬇️ Installation
1. Download [latest Release version](https://github.com/pashagta555/playerok-universal-english-mod/releases/latest) and unpack it to any place convenient for you
2. Make sure that you have **Python version 3.12.x** installed (the bot is not guaranteed to work on other versions). If not installed, do so by following the link https://www.python.org/downloads/release/python-31210/ (when installing, click on the item `Add to PATH`)
...
4. To launch the bot, open the `start.bat` launcher
...
| `ON_PLAYEROK_BOT_INIT` | When initializing (launching) the Playerok bot | `PlayerokBot` |
| `ON_TELEGRAM_BOT_INIT` | When initializing (launching) the Telegram bot | `TelegramBot` |
...
| `EventTypes.NEW_DEAL` | A new deal has been created (when the buyer has paid for the item) | `PlayerokBot`, `NewDealEvent` |
...
| `EventTypes.DEAL_ROLLED_BACK` | Seller issued a refund deals | `PlayerokBot`, `DealRolledBackEvent` |
...
| `EventTypes.DEAL_STATUS_CHANGED` | Deals status changed | `PlayerokBot`, `DealStatusChangedEvent` |
| `EventTypes.ITEM_PAID` | The user paid for item | `PlayerokBot`, `ItemPaidEvent` |
| `EventTypes.ITEM_SENT` | Item sent (seller confirmed deals completed) | `PlayerokBot`, `ItemSentEvent` |
...
</br>Module is a folder that contains important components. You can study the structure of a module based on [template module](.templates/forms_module), but you should understand that this is just an example made by us.
...
| `TELEGRAM_BOT_ROUTERS` | `list[Router]` | This array specifies the routers of the modular Telegram bot |
...
Also, if a module requires additional dependencies, it must contain a dependency file **requirements.txt**, which will be downloaded themselves when all bot modules are loaded.

#### 🔧 Example content:
Please note that the metadata was placed in a separate file `meta.py`, but is imported into `__init__.py`.
...
### 📝 Customized wrappers for configuration files and data files
...
#### How does it work?
Let's say you want to create a configuration file in your module, for this you will need to create a `settings.py` file in the root of the module folder.
The contents of `settings.py` should be something like this:
  ```python
  import os
...
need_restore=True, # whether the config needs to be restored
      default={
          "bool_param": True,
          "str_param": "qwerty",
          "int_param": 123
...
</br>The data file is designed in exactly the same way, but it is needed to store information collected by the script itself, and not specified by users.
...
The contents of `data.py` should be something like this:
  ```python
  import os
  from data import (
      Data as data,
...
In order to do this, you must first obtain the UUID of the currently running module, which is generated during its initialization.
...
await disable_module(get_module().uuid) # disables a module
...
</br>The functionality of the Telegram bot is written in the aiogram 3 library, the system for implementing custom functionality of the Telegram bot works on the basis of routers that merge with the main, main router of the bot.
And the way they merge together, complications can arise if, for example, Callback data has an identical name. Therefore, after writing the Telegram bot functionality for the module, it is better to rename
