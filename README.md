...
[![python](https://img.shields.io/badge/python-3.12.x-yellow?style=for-the-badge&logo=python&link=https%3A%2F%2Fimg.shields.io%2Fbadge%2Ftelegram-%25D 0%25BA%25D0%25B0%25D0%25BD%25D0%25B0%25D0%25BB-blue%3Fstyle%3Dfor-the-badge%26logo%3Dtelegram)](https://www.python.org/downloads/release/python-3119/)
...
2. Make sure that you have **Python version 3.12.x** installed (the bot is not guaranteed to work on other versions). If not installed, do so by going to https://www.python.org/downloads/release/python-31210/ (when installing, click on `Add to PATH`)
...
<details>
  ...
  | `ON_PLAYEROK_BOT_INIT` | When initializing (launching) the Playerok bot | `PlayerBot` |
  ...
  | `EventTypes.NEW_REVIEW` | New review of the deal | `PlayerBot`, `NewReviewEvent` || `EventTypes.DEAL_CONFIRMED` | Deal confirmed | `PlayerokBot`, `DealConfirmedEvent` |
  | `EventTypes.DEAL_ROLLED_BACK` | The seller issued a refund of the transaction | `PlayerBot`, `DealRolledBackEvent` |
...
<details>
  ...
  #### 🔧 Example content:
  Please note that the metadata was placed in a separate `meta.py` file, but is imported into `__init__.py`.
...
<details>
  ...
  The configuration file is specified using the `SettingsFile` dataclass, which in turn is passed to the `DATA` array.
  ...
  For example, you want to create a data file in your module, for this you will need to create a `data.py` file in the root of the module folder.
  ...
  await disable_module(get_module().uuid) # disables the module
  await enable_module(get_module().uuid) # enables the module
...
<details>
  ...
  And the way they merge howlingHowever, complications may arise if, for example, Callback data has an identical name. Therefore, after writing the Telegram bot functionality for the module, it is better to rename
...
## 🔗 Useful links
- Developer: https://github.com/alleexxeeyy (the profile has up-to-date links to all contacts for communication)
