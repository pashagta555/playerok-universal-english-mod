...
- Module system (plugins connectable to the bot)
...
- `!seller` command to call seller to chat
...
2. Make sure you have **Python version 3.11.x** installed (bot operation is not guaranteed on other versions). If not installed, do so by following the link https://www.python.org/downloads/release/python-3119/ (during installation, click on `Add to PATH`)
...
[Having installation problems? Click me](https://telegra.ph/FunPay-Universal--chastye-oshibki-i-ih-resheniya-08-26)

## 📚 For Developers

The modular system helps integrate additional functionality into the bot, created by enthusiasts. Essentially, it's the same as plugins, but in a more convenient format.
...
<details>
  ...
  Events that are executed on certain bot actions.
  ...
  Events received in the event listener in Playerok bot.
  ...
  | `EventTypes.DEAL_ROLLED_BACK` | Seller processed deal refund | `PlayerokBot`, `DealRolledBackEvent` |
  | `EventTypes.DEAL_HAS_PROBLEM` | User reported problem in deal | `PlayerokBot`, `DealHasProblemEvent` |
...
<details>
  ...
  </br>A module is a folder containing important components. You can study the module structure based on the [template module](.templates/forms_module), but understand that this is just an example we created.

  Required handler constants:
  | Constant | Type | Description |
  |-----------|-----|----------|
  | `BOT_EVENT_HANDLERS` | `dict[str, list[Any]]` | This dictionary sets bot event handlers |
  | `PLAYEROK_EVENT_HANDLERS` | `dict[EventTypes, list[Any]` | This dictionary sets Playerok event handlers |
  | `TELEGRAM_BOT_ROUTERS` | `list[Router]` | This array sets modular Telegram bot routers  |
  ...
  Also, if the module requires additional dependencies, it should have a **requirements.txt** dependency file, which will be automatically downloaded when loading all bot modules.
...
<details>
  ...
  ### 📝 Configured wrappers for configuration files and data files
  ...
  The bot has already configured classes in files [`settings.py`](settings.py) and [`data.py`](data.py)
  ...
  The configuration file is set using the dataclass `SettingsFile`, which in turn is passed to the `DATA` array.
  ...
  When setting a new value to the config, it is immediately written to its file. Also, when getting, current data is taken from the file.

  Description of `SettingsFile` dataclass arguments:
  | Argument | Description |
  |----------|----------|
  | `name` | Configuration file name that we'll use when getting and writing |
  | `path` | Path to configuration file |
  | `need_restore` | Whether to restore config? Let's say new data was added to the default config value, but it's missing in an **already created** configuration file. If the parameter is enabled, the script will compare current config data with the specified defaults, and if a key exists in the default value but not in current data, it will be automatically added to the config. Also, if the key value type in the default config doesn't match the existing one (e.g., **string** type in file, but **numeric** in default value), this key in the current config will be replaced with the default value |
  ...
  Here everything is similar to the configuration file, only serves a different purpose.
...
<details>
  ...
  </br>Telegram bot functionality is written on aiogram 3 library, the system for integrating custom Telegram bot functionality works based on routers that merge with the main, primary bot router.
  ...
  this data in a unique way so it doesn't match the names of the main bot or additional connected modules.
