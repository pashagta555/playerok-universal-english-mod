...
def setup_logger(log_file: str='logs/latest.log'):
    ...
    formatter = ShortLevelFormatter(LOG_FORMAT, datefmt='%d.%m.%Y %H:%M:%S', reset=True, log_colors={'DEBUG': 'light_blue', 'INFO': 'light_green', 'WARNING': 'yellow', 'ERROR': 'bold_red', 'CRITICAL': 'red'}, style='%')
    ...
    file_handler.setFormatter(StripColorFormatter('[%(asctime)s] %(levelname)-1s · %(name)-20s %(message)s', datefmt='%d.%m.%Y %H:%M:%S'))
