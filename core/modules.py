...
async def enable_module(module_uuid: UUID) -> bool:
    """
    Enables a module and registers its handlers.
...
async def reload_module(module_uuid: str):
    """
    Reloads a module (unloads and imports again).
...
def _format_string(count: int):
    ...
    return f"{Fore.LIGHTWHITE_EX}{n}{Fore.WHITE} {word} connected"


async def connect_modules(modules: list[Module]):
    """
    Connects (enables) loaded modules.
