...
async def enable_module(module_uuid: UUID) -> bool:
    """
    Enables module and adds its handlers.

    :param module_uuid: Module UUID.
    :type module_uuid: `uuid.UUID`

    :return: True if module was enabled. False if not enabled.
    :rtype: `bool`
    """
    try:
        ...
        logger.error(f"{Fore.LIGHTRED_EX}Error enabling module {module_uuid}: {Fore.WHITE}{e}")
        return False
...
async def disable_module(module_uuid: UUID) -> bool:
    """ 
    Disables module and removes its handlers.
    
    :param module_uuid: Module UUID.
    :type module_uuid: `uuid.UUID`

    :return: True if module was disabled. False if not disabled.
    :rtype: `bool`
    """
    try:
        module = get_module_by_uuid(module_uuid)
        ...
        logger.error(f"{Fore.LIGHTRED_EX}Error disabling module {module_uuid}: {Fore.WHITE}{e}")
        return False


async def reload_module(module_uuid: str):
    """
    Reloads module (unloads and imports again).
    
    :param module_uuid: Module UUID.
    :type module_uuid: `uuid.UUID`

    :return: True if module was reloaded. False if not reloaded.
    :rtype: `bool`
    """
    try:
        module = get_module_by_uuid(module_uuid)
...
def _format_string(count: int):
    ...
    if last_num == 1: return f"Connected {Fore.LIGHTCYAN_EX}{count} module"
    elif 2 <= last_num <= 4: return f"Connected {Fore.LIGHTCYAN_EX}{count} modules"
    elif 5 <= last_num <= 9 or last_num == 0: return f"Connected {Fore.LIGHTCYAN_EX}{count} modules"
