...
from core.handlers import register_bot_event_handlers, register_playerok_event_handlers, remove_bot_event_handlers, remove_playerok_event_handlers, call_bot_event
...
def get_module_by_uuid(module_uuid: UUID) -> Module | None:
    ...
    """
    try:
        return [module for module in loaded_modules if module.uuid == module_uuid][0]
    except:
        return None
...
async def enable_module(module_uuid: UUID) -> bool:
    ...
    try:
        ...
        logger.info(f'Module {Fore.LIGHTWHITE_EX}{module.meta.name} {Fore.WHITE}turned on')
        return True
    except Exception as e:
        logger.error(f'{Fore.LIGHTRED_EX}Error when turning on the module {module_uuid}: {Fore.WHITE}{e}')
        return False
...
async def disable_module(module_uuid: UUID) -> bool:
    ...
    try:
        ...
        logger.info(f'Module {Fore.LIGHTWHITE_EX}{module.meta.name} {Fore.WHITE}off')
        return True
    except Exception as e:
        logger.error(f'{Fore.LIGHTRED_EX}Error when turning off the module {module_uuid}: {Fore.WHITE}{e}')
        return False
...
async def reload_module(module_uuid: str):
    ...
    try:
        ...
        await _enable_module(module)
        logger.info(f'Module {Fore.LIGHTWHITE_EX}{module.meta.name} {Fore.WHITE}rebooted')
        return True
    except Exception as e:
        logger.error(f'{Fore.LIGHTRED_EX}Error when rebooting module {module_uuid}: {Fore.WHITE}{e}')
        return False
    ...
    for name in os.listdir(modules_path):
        ...
        if os.path.isdir(module_path) and '__init__.py' in os.listdir(module_path):
            try:
                ...
                module_data = Module(uuid.uuid4(), enabled=False, meta=ModuleMeta(module.PREFIX, module.VERSION, module.NAME, module.DESCRIPTION, module.AUTHORS, module.LINKS), bot_event_handlers=bot_event_handlers, playerok_event_handlers=playerok_event_handlers, telegram_bot_routers=telegram_bot_routers, _dir_name=name)
                modules.append(module_data)
            except Exception as e:
                logger.error(f'{Fore.LIGHTRED_EX}Error loading module {name}: {Fore.WHITE}{traceback.format_exc()}')
    return modules

def _format_string(count: int):
    last_num = int(str(count)[-1])
    if last_num == 1:
        return f'Connected {Fore.LIGHTWHITE_EX}{count} module'
    elif 2 <= last_num <= 4:
        return f'Connected {Fore.LIGHTWHITE_EX}{count} module'
    elif 5 <= last_num <= 9 or last_num == 0:
        return f'Connected {Fore.LIGHTWHITE_EX}{count} modules'
