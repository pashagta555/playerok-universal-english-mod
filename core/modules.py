...
async def enable_module(module_uuid: UUID) -> bool:
    ...
    :return: True if module was enabled. False if it wasn't enabled.
    :rtype: `bool`
    """
    try:
...
async def disable_module(module_uuid: UUID) -> bool:
    ...
    :return: True if module was disabled. False if it wasn't disabled.
    :rtype: `bool`
    """
    try:
        module = get_module_by_uuid(module_uuid)
...
async def reload_module(module_uuid: str):
    ...
    :return: True if module was reloaded. False if it wasn't reloaded.
    :rtype: `bool`
    """
    try:
        module = get_module_by_uuid(module_uuid)
...
def _format_string(count: int):
    ...
    if count == 1: return f"Connected {Fore.LIGHTCYAN_EX}{count} module"
    else: return f"Connected {Fore.LIGHTCYAN_EX}{count} modules"
