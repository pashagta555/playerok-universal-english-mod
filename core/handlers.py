...
def get_bot_event_handlers() -> dict[str, list[callable]]:
    ...
    :return: Dictionary with events and handler lists.
...
def set_bot_event_handlers(data: dict[str, list[callable]]):
    ...
    :param data: Dictionary with event names and handler lists.
...
def add_bot_event_handler(event: str, handler: callable, index: int | None = None):
    ...
    :param index: Index in handlers list, _optional_.
...
def register_bot_event_handlers(handlers: dict[str, list[callable]]):
    """
    Registers bot event handlers (adds given handlers if missing).
...
def remove_bot_event_handlers(handlers: dict[str, list[callable]]):
    """
    Removes given bot handlers.

    :param handlers: Dictionary with events and handler lists.
...
def set_playerok_event_handlers(data: dict[EventTypes, list[callable]]):
    ...
    :param data: Dictionary with events and handler lists.
...
def register_playerok_event_handlers(handlers: dict[EventTypes, list[callable]]):
    """
    Registers Playerok event handlers (adds given handlers if missing).
...
def remove_playerok_event_handlers(handlers: dict[EventTypes, list[callable]]):
    """
    Removes given Playerok handlers.
...
async def call_bot_event(event: str, args: list = [], func = None):
    ...
    :param func: Specific function to call for the event, _optional_.
    ...
    for handler in handlers:
        try:
            ...
            logger.error(f"{Fore.LIGHTRED_EX}Error while handling handler «{handler.__module__}.{handler.__qualname__}» for bot event «{event}»: {Fore.WHITE}{e}")
...
async def call_playerok_event(event: EventTypes, args: list = []):
    ...
    for handler in handlers:
        try:
            ...
            logger.error(f"{Fore.LIGHTRED_EX}Error while handling handler «{handler.__module__}.{handler.__qualname__}» for Playerok event «{event.name}»: {Fore.WHITE}{e}")