...
_bot_event_handlers: dict[str, list[callable]] = {'ON_MODULE_CONNECTED': [], 'ON_MODULE_ENABLED': [], 'ON_INIT': [], 'ON_PLAYEROK_BOT_INIT': [], 'ON_TELEGRAM_BOT_INIT': []}
_playerok_event_handlers: dict[EventTypes, list[callable]] = {EventTypes.CHAT_INITIALIZED: [], EventTypes.NEW_MESSAGE: [], EventTypes.NEW_DEAL: [], EventTypes.NEW_REVIEW: [], EventTypes.DEAL_CONFIRMED: [], EventTypes.DEAL_CONFIRMED_AUTOMATICALLY: [], EventTypes.DEAL_ROLLED_BACK: [], EventTypes.DEAL_HAS_PROBLEM: [], EventTypes.DEAL_PROBLEM_RESOLVED: [], EventTypes.DEAL_STATUS_CHANGED: [], EventTypes.ITEM_PAID: [], EventTypes.ITEM_SENT: []}
...
def add_bot_event_handler(event: str, handler: callable, index: int | None=None):
    ...
    :param event: Name события, для которого добавляется хендлер.
    :type event: `str`
...
async def call_bot_event(event: str, args: list=[], func=None):
    ...
    for handler in handlers:
        try:
            ...
            logger.error(f'{Fore.LIGHTRED_EX}Error processing handler «{handler.__module__}.{handler.__qualname__}» for a bot event «{event}»: {Fore.WHITE}{e}')
