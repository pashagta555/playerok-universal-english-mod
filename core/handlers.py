...
def add_playerok_event_handler(event: EventTypes, handler: callable, index: int | None = None):
    ...
    :param event: The event for which a handler is added.
...
async def call_bot_event(event: str, args: list = [], func = None):
    ...
    :param func: The function for which the event needs to be called (if only one specific one needs to be called) is _optional_.
