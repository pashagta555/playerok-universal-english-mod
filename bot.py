...
def check_and_configure_config():
    ...
    while not config["playerok"]["api"]["token"]:
        while not config["playerok"]["api"]["token"]:
            print(f"\n{Fore.WHITE}Enter your Playerok account {Fore.LIGHTBLUE_EX}token{Fore.WHITE}. You can find it in Cookie data, use the Cookie-Editor extension."
        ...
        while not config["playerok"]["api"]["user_agent"]:
            print(f"\n{Fore.WHITE}Enter your browser's {Fore.LIGHTMAGENTA_EX}User Agent{Fore.WHITE}. You can copy it from the site {Fore.LIGHTWHITE_EX}https://whatmyuseragent.com. Or you can skip this parameter by pressing Enter."
            ...
            if not user_agent:
                print(f"\n{Fore.YELLOW}You skipped entering User Agent. Note that in this case the bot may work unstable.")
                break
        ...
        while not config["playerok"]["api"]["proxy"]:
            print(f"\n{Fore.WHITE}Enter {Fore.LIGHTBLUE_EX}IPv4 Proxy{Fore.WHITE} in format user:password@ip:port or ip:port if it's without authentication. If you don't know what this is, or don't want to set up a proxy - skip this parameter by pressing Enter."
    ...
    while not config["telegram"]["api"]["token"]:
        print(f"\n{Fore.WHITE}Enter your Telegram bot's {Fore.CYAN}token{Fore.WHITE}. The bot needs to be created with @BotFather."
    ...
    while not config["telegram"]["bot"]["password"]:
        print(f"\n{Fore.WHITE}Create and enter a {Fore.YELLOW}password for your Telegram bot{Fore.WHITE}. The bot will request this password on each new attempt by another user to interact with your Telegram bot."
        ...
        if is_password_valid(password):
            ...
            print(f"\n{Fore.LIGHTRED_EX}Your password doesn't meet requirements. Make sure it matches the format and is not too simple, then try again.")
    ...
    if not is_pl_account_working():
        print(f"\n{Fore.LIGHTRED_EX}Failed to connect to your Playerok account. Please make sure you have the correct token and enter it again.")
        config["playerok"]["api"]["token"] = ""
        config["playerok"]["api"]["user_agent"] = ""
        config["playerok"]["api"]["proxy"] = ""
        sett.set("config", config)
    ...
    if not is_tg_bot_exists():
        print(f"\n{Fore.LIGHTRED_EX}Failed to connect to your Telegram bot. Please make sure you have the correct token and enter it again.")
