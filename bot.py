...
def check_and_configure_config():
    ...
    while not config["playerok"]["api"]["token"]:
        ...
        while not config["playerok"]["api"]["user_agent"]:
            print(f"\n{Fore.WHITE}Enter the {Fore.LIGHTMAGENTA_EX}User Agent {Fore.WHITE}of your browser. You can copy it from {Fore.LIGHTWHITE_EX}https://whatmyuseragent.com. Or you can skip this parameter by pressing Enter."
        ...
        while not config["playerok"]["api"]["proxy"]:
            print(f"\n{Fore.WHITE}Enter {Fore.LIGHTBLUE_EX}IPv4 Proxy {Fore.WHITE}in the format user:password@ip:port or ip:port if it's without authentication. If you don't know what this is, or don't want to set up a proxy - skip this parameter by pressing Enter."
    ...
    while not config["telegram"]["bot"]["password"]:
        ...
        if is_password_valid(password):
            ...
            print(f"\n{Fore.LIGHTRED_EX}Your password is not suitable. Make sure it matches the format and is not easy and try again.")
