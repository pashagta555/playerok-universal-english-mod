...
def check_and_configure_config():
    ...
    while not config["playerok"]["api"]["token"]:
        while not config["playerok"]["api"]["token"]:
            print(
                f"\n{Fore.WHITE}Enter your {Fore.LIGHTBLUE_EX}Playerok account token{Fore.WHITE}. "
                f"You can get it from cookies using a browser extension like Cookie-Editor."
            ...
            if is_token_valid(token):
                ...
                print(
                    f"\n{Fore.LIGHTRED_EX}It looks like the token is invalid. "
        ...
        while not config["playerok"]["api"]["user_agent"]:
            print(
                ...
                f"Or press Enter to skip this field."
            ...
            if not user_agent:
                print(f"\n{Fore.YELLOW}User-Agent was skipped. Note: without it the bot may work less reliably.")
                break
            if is_user_agent_valid(user_agent):
                ...
                print(
                    f"\n{Fore.LIGHTRED_EX}It looks like the User-Agent is invalid. "
                    f"Make sure it has no Cyrillic characters and try again."
                )
        ...
        while not config["playerok"]["api"]["proxy"]:
            print(
                ...
                f"Format: user:password@ip:port or ip:port (without auth). "
            ...
            if is_proxy_valid(proxy):
                ...
                print(
                    f"\n{Fore.LIGHTRED_EX}It looks like the proxy is invalid. "
                    f"Please check the format and try again."
                )
    ...
    while not config["telegram"]["api"]["token"]:
        while not config["telegram"]["api"]["token"]:
            print(
                f"\n{Fore.WHITE}Enter your {Fore.CYAN}Telegram bot token{Fore.WHITE}. You can create a bot via @BotFather."
    ...
    while not config["telegram"]["bot"]["password"]:
        print(
            ...
            f"The bot will ask for this password when a new user tries to access it."
            f"\n  {Fore.WHITE}· Password length must be between 6 and 64 characters."
        )
        ...
        if is_password_valid(password):
            ...
            print(f"\n{Fore.LIGHTRED_EX}Invalid password. Please check requirements and try again.")

    if config["playerok"]["api"]["proxy"] and not is_proxy_working(config["playerok"]["api"]["proxy"]):
        print(
            f"\n{Fore.LIGHTRED_EX}The proxy for the Playerok account does not work. "
            f"Please check it and enter a valid one."
        )
    ...
    if not is_pl_account_working():
        print(
            ...
            f"Please make sure the token is valid and try again."
        )
        config["playerok"]["api"]["token"] = ""
        config["playerok"]["api"]["user_agent"] = ""
        config["playerok"]["api"]["proxy"] = ""
    ...
    if is_pl_account_banned():
        print(
            ...
            f"The bot cannot run on a blocked account."
        )
        config["playerok"]["api"]["token"] = ""
        config["playerok"]["api"]["user_agent"] = ""
        config["playerok"]["api"]["proxy"] = ""
    ...
    if config["telegram"]["api"]["proxy"] and not is_proxy_working(
        ...
        print(
            f"{Fore.LIGHTRED_EX}\nThe proxy for the Telegram bot does not work. "
            f"Please check it and enter a valid one."
        )
    ...
    if not is_tg_bot_exists():
        print(
            ...
            f"If you are in Russia, you may need to configure a proxy or use VPN due to Telegram restrictions."
        )
        config["telegram"]["api"]["token"] = ""
        config["telegram"]["api"]["proxy"] = ""
        sett.set("config", config)
...
if __name__ == "__main__":
    try:
        ...
        traceback.print_exc()
        print(
            f"\n{Fore.LIGHTRED_EX}Your bot encountered an unexpected error and stopped."
            f"\n\n{Fore.WHITE}Try to find the issue in our troubleshooting article with the most common errors.",
