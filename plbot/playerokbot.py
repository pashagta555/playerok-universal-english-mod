...
class PlayerokBot:
    ...
    def __init__(self):
        ...
        self.account = self.playerok_account = Account(cookies=self.config['playerok']['api']['cookies'], user_agent=self.config['playerok']['api']['user_agent'], proxy=self.config['playerok']['api']['proxy'] or None, requests_timeout=self.config['playerok']['api']['requests_timeout']).get()
    ...
    def refresh_account(self):
        try:
            ...
            logger.error(f'{Fore.LIGHTRED_EX}Error during update аккаунта: {Fore.WHITE}{e}')
    ...
    def check_banned(self):
        try:
            ...
            if user.is_blocked:
                logger.critical('')
                logger.critical(f'{Fore.LIGHTRED_EX}Ваш Playerok аккаунт был заблокирован! К сожалению, я не могу продолжать работу on заблокированном аккаунте...')
                logger.critical(f'Напишите в тех. поддержку Playerok, чтобы узнать причину бана и как можно быстрее решить эту проблему.')
                logger.critical('')
                shutdown()
        except Exception as e:
            logger.error(f'{Fore.LIGHTRED_EX}Ошибка при проверке on блокировку: {Fore.WHITE}{e}')
    ...
    def msg(self, message_name: str, messages_config_name: str='messages', messages_data: dict=DATA, **kwargs) -> str | None:
        ...
        try:
            ...
            pass
        return f'Failed to receive message {message_name}'

    def _do_call_event(self, latest_event_time, event_interval):
        if not latest_event_time:
            return True
        if datetime.now() >= datetime.fromisoformat(latest_event_time) + timedelta(seconds=event_interval):
            return True
        return False

    def send_message(self, chat_id: str, text: str | None=None, photo_file_paths: list[str]=[], mark_chat_as_read: bool=None, exclude_watermark: bool=False, max_attempts: int=3) -> ChatMessage | None:
        """
        ...
        for _ in range(max_attempts):
            try:
                ...
                mess = self.account.send_message(chat_id=chat_id, text=text, photo_file_paths=photo_file_paths, mark_chat_as_read=mark_chat_as_read)
                return mess
            except Exception as e:
                err = e
                break
        ...
        logger.error(f'{Fore.LIGHTRED_EX}Error sending message «{msg}» {Fore.LIGHTRED_EX}in chat {chat_id}{Fore.LIGHTRED_EX}: {Fore.WHITE}{err}')
    ...
    def get_my_items(self, count: int=-1, game_id: str | None=None, category_id: str | None=None, statuses: list[ItemStatuses] | None=None) -> list[ItemProfile]:
        """
        ...
        try:
            ...
            next_cursor = None
            while True:
                itm_list = user.get_items(after_cursor=next_cursor, game_id=game_id, category_id=category_id)
    ...
    def bump_item(self, item: ItemProfile | MyItem):
        try:
            ...
            included = any((any((phrase.lower() in item.name.lower() or item.name.lower() == phrase.lower() for phrase in included_item)) for included_item in self.auto_bump_items['included']))
            excluded = any((any((phrase.lower() in item.name.lower() or item.name.lower() == phrase.lower() for phrase in excluded_item)) for excluded_item in self.auto_bump_items['excluded']))
            if self.config['playerok']['auto_bump_items']['all'] and (not excluded) or (not self.config['playerok']['auto_bump_items']['all'] and included):
                ...
                logger.info(f'{Fore.LIGHTWHITE_EX}«{name_frmtd}» {Fore.WHITE}— {Fore.YELLOW}raised. {Fore.WHITE}Position: {Fore.LIGHTWHITE_EX}{item.sequence} {Fore.WHITE}→ {Fore.YELLOW}1')
        except Exception as e:
            logger.error(f'{Fore.LIGHTRED_EX}Error when picking up an item «{name_frmtd}»: {Fore.WHITE}{e}')
    ...
    def bump_items(self):
        try:
            ...
            logger.error(f'{Fore.LIGHTRED_EX}Ошибка при поднятии items: {Fore.WHITE}{e}')
    ...
    def restore_item(self, item: Item | MyItem | ItemProfile):
        try:
            ...
            excluded = any((any((phrase.lower() in item.name.lower() or item.name.lower() == phrase.lower() for phrase in excluded_item)) for excluded_item in self.auto_restore_items['excluded']))
            if self.config['playerok']['auto_restore_items']['all'] and (not excluded) or (not self.config['playerok']['auto_restore_items']['all'] and included):
                if not isinstance(item, MyItem):
                    try:
                        item = self.account.get_item(item.id)
                    except:
                ...
                time.sleep(1)
                statuses = self.account.get_item_priority_statuses(item.id, item.raw_price)
                pr_status = next((st for st in statuses if st.type == PriorityTypes.DEFAULT or st.price == 0), statuses[0])
                time.sleep(1)
                ...
                if new_item.status in (ItemStatuses.PENDING_APPROVAL, ItemStatuses.APPROVED):
                    logger.info(f'{Fore.LIGHTWHITE_EX}«{name_frmtd}» {Fore.WHITE}— {Fore.YELLOW}the item has been restored')
                else:
                    logger.error(f'{Fore.LIGHTRED_EX}Failed to restore item «{name_frmtd}». His current status: {Fore.WHITE}{new_item.status.name}')
        except Exception as e:
            logger.error(f'{Fore.LIGHTRED_EX}Error when restoring an item «{name_frmtd}»: {Fore.WHITE}{e}')
    ...
    def restore_expired_items(self):
        try:
            ...
            logger.error(f'{Fore.LIGHTRED_EX}Ошибка при восстановлении истёкших items: {Fore.WHITE}{e}')
    ...
    def request_withdrawal(self) -> bool:
        try:
            ...
            if balance <= 500:
                raise Exception('Слишком маленький баланс. Транзакция должна быть on сумму от 500₽')
            ...
            self.account.request_withdrawal(provider=provider, account=account, value=balance, sbp_bank_member_id=sbp_bank_member_id)
            logger.info(f"{Fore.LIGHTWHITE_EX}{balance or '?'}₽ {Fore.WHITE}— {Fore.YELLOW}транзакция on вывод создана")
            return True
        except Exception as e:
            logger.error(f'{Fore.LIGHTRED_EX}Ошибка при создании транзакции on вывод {balance}₽: {Fore.WHITE}{e}')
        return False
    ...
    def log_new_message(self, message: ChatMessage, chat: Chat):
        ...
        ch_header = f'New chat message с {chat_user}:'
        ...
        if message.text is not None:
            ...
            text = f'{Fore.LIGHTMAGENTA_EX}Image {Fore.WHITE}({message.file.url})'
    ...
    def log_new_review(self, deal: ItemDeal):
        logger.info(f'{Fore.YELLOW}───────────────────────────────────────')
        logger.info(f'{Fore.YELLOW}New review по сделке {deal.id}:')
        logger.info(f" · Assessment: {Fore.LIGHTYELLOW_EX}{'★' * deal.review.rating or 5} ({deal.review.rating or 5})")
    ...
    def log_deal_status_changed(self, deal: ItemDeal, status_frmtd: str='Unknown'):
        ...
        logger.info(f'{Fore.WHITE}Status сделки {Fore.LIGHTWHITE_EX}{deal.id} {Fore.WHITE}changed:')
    ...
    def log_new_problem(self, deal: ItemDeal):
        logger.info(f'{Fore.YELLOW}───────────────────────────────────────')
        logger.info(f'{Fore.YELLOW}New complaint in the transaction {deal.id}:')
    ...
    async def _on_playerok_bot_init(self):
        ...
        def bump_items_loop():
            while True:
                if self.config['playerok']['auto_bump_items']['enabled'] and self._do_call_event(self.latest_events_times['auto_bump_items'], self.config['playerok']['auto_bump_items']['interval']):
                    self.bump_items()
                time.sleep(3)

        def withdrawal_loop():
            while True:
                if self.config['playerok']['auto_withdrawal']['enabled'] and self._do_call_event(self.latest_events_times['auto_withdrawal'], self.config['playerok']['auto_withdrawal']['interval']):
    ...
    async def _on_new_message(self, event: NewMessageEvent):
        ...
        if self.config['playerok']['tg_logging']['enabled'] and (self.config['playerok']['tg_logging']['events']['new_user_message'] or self.config['playerok']['tg_logging']['events']['new_system_message']):
            do = False
            if self.config['playerok']['tg_logging']['events']['new_user_message'] and (not is_support_chat) or (self.config['playerok']['tg_logging']['events']['new_system_message'] and is_support_chat):
                do = True
            if do:
                ...
                asyncio.run_coroutine_threadsafe(get_telegram_bot().log_event(text=log_text(title=f'💬 Новое сообщение в <a href="https://playerok.com/chats/{event.chat.id}">чате</a>', text=text.strip()), kb=log_new_mess_kb(event.message.user.username)), get_telegram_bot_loop())
        if not is_support_chat and event.message.text is not None:
            ...
            if str(event.message.text).lower() in ('!commands', '!commands'):
                self.send_message(event.chat.id, self.msg('cmd_commands'))
            elif str(event.message.text).lower() in ('!seller', '!seller'):
                asyncio.run_coroutine_threadsafe(get_telegram_bot().call_seller(event.message.user.username, event.chat.id), get_telegram_bot_loop())
    ...
    async def _on_new_review(self, event: NewReviewEvent):
        ...
        if self.config['playerok']['tg_logging']['enabled'] and self.config['playerok']['tg_logging']['events']['new_review']:
            asyncio.run_coroutine_threadsafe(get_telegram_bot().log_event(text=log_text(title=f'💬✨ Новый отзыв по <a href="https://playerok.com/deal/{event.deal.id}">сделке</a>', text=f"<b>Оценка:</b> {'⭐' * event.deal.review.rating}\n<b>Оставил:</b> {event.deal.review.creator.username}\n<b>Текст:</b> {event.deal.review.text}\n<b>Дата:</b> {datetime.fromisoformat(event.deal.review.created_at).strftime('%d.%m.%Y %H:%M:%S')}"), kb=log_new_mess_kb(event.deal.user.username)), get_telegram_bot_loop())
        self.send_message(event.chat.id, self.msg('new_review', deal_id=event.deal.id, deal_item_name=event.deal.item.name, deal_item_price=event.deal.item.price, review_rating=event.deal.review.rating))

    async def _on_new_problem(self, event: ItemPaidEvent):
        if event.deal.user.id == self.account.id:
            return
        self.log_new_problem(event.deal)
        if self.config['playerok']['tg_logging']['enabled'] and self.config['playerok']['tg_logging']['events']['new_problem']:
            asyncio.run_coroutine_threadsafe(get_telegram_bot().log_event(text=log_text(title=f'🤬 Новая жалоба в <a href="https://playerok.com/deal/{event.deal.id}">сделке</a>', text=f'<b>Покупатель:</b> {event.deal.user.username}\n<b>Предмет:</b> {event.deal.item.name}'), kb=log_new_mess_kb(event.deal.user.username)), get_telegram_bot_loop())
    ...
    async def _on_new_deal(self, event: NewDealEvent):
        ...
        try:
            ...
            pass
        self.cached_orders[event.deal.id] = {'id': event.deal.id, 'price': event.deal.item.price, 'status': event.deal.status.name, 'date': datetime.now(pytz.timezone('Europe/Moscow')).isoformat(), 'item_id': event.deal.item.id, 'item_name': event.deal.item.name}
        self.log_new_deal(event.deal)
        if self.config['playerok']['tg_logging']['enabled'] and self.config['playerok']['tg_logging']['events']['new_deal']:
            asyncio.run_coroutine_threadsafe(get_telegram_bot().log_event(text=log_text(title=f'📋 Новая <a href="https://playerok.com/deal/{event.deal.id}">сделка</a>', text=f"<b>Покупатель:</b> {event.deal.user.username}\n<b>Предмет:</b> {event.deal.item.name or '-'}\n<b>Сумма:</b> {event.deal.item.price or '?'}₽"), kb=log_new_deal_kb(event.deal.user.username, event.deal.id)), get_telegram_bot_loop())
                ...
                for phrase in auto_delivery['keyphrases']:
                    if phrase.lower() in (event.deal.item.name or '').lower() or (event.deal.item.name or '').lower() == phrase.lower():
                        ...
                        if piece:
                            ...
                            if mess:
                                logger.info(f"{Fore.YELLOW}To the buyer {Fore.LIGHTYELLOW_EX}{event.deal.user.username or '?'} {Fore.YELLOW}goods issued {Fore.LIGHTYELLOW_EX}«{good}»{Fore.YELLOW}. Remainder: {Fore.LIGHTYELLOW_EX}{len(goods) - 1}")
                            ...
                            if msg:
                                ...
                                if mess:
                                    logger.info(f"{Fore.YELLOW}To the buyer {Fore.LIGHTYELLOW_EX}{event.deal.user.username or '?'} {Fore.YELLOW}отправлено сообщение авто-выдачи {Fore.LIGHTYELLOW_EX}«{msg}»")
                        break
        if self.config['playerok']['auto_complete_deals']['enabled']:
            ...
            excluded = any((any((phrase.lower() in event.deal.item.name.lower() or event.deal.item.name.lower() == phrase.lower() for phrase in excluded_item)) for excluded_item in self.auto_complete_deals['excluded']))
            if self.config['playerok']['auto_complete_deals']['all'] and (not excluded) or (not self.config['playerok']['auto_complete_deals']['all'] and included):
                self.account.update_deal(event.deal.id, ItemDealStatuses.SENT)
                logger.info(f'{Fore.YELLOW}Deal confirmed автоматически {Fore.WHITE}(https://playerok.com/deal/{event.deal.id})')
    ...
    async def _on_deal_status_changed(self, event: DealStatusChangedEvent):
        ...
        if event.deal.status is ItemDealStatuses.PAID:
            ...
            status_frmtd = 'Buyer подтвердил сделку'
        elif event.deal.status is ItemDealStatuses.ROLLED_BACK:
            status_frmtd = 'Return'
        self.log_deal_status_changed(event.deal, status_frmtd)
        if self.config['playerok']['tg_logging']['enabled'] and self.config['playerok']['tg_logging']['events']['deal_status_changed']:
            asyncio.run_coroutine_threadsafe(get_telegram_bot().log_event(log_text(title=f'🔄️📋 Статус <a href="https://playerok.com/deal/{event.deal.id}/">сделки</a> изменился', text=f'<b>Новый статус:</b> {status_frmtd}')), get_telegram_bot_loop())
    ...
    async def run_bot(self):
        logger.info('')
        logger.info(f'{Fore.YELLOW}Playerok bot is running and active')
        logger.info('')
        logger.info(f'{Fore.YELLOW}───────────────────────────────────────')
        logger.info(f'{Fore.YELLOW}Account information:')
        ...
        if profile.balance:
            ...
            logger.info(f'   · Waiting: {Fore.LIGHTWHITE_EX}{profile.balance.pending_income}₽')
            logger.info(f'   · Frozen: {Fore.LIGHTWHITE_EX}{profile.balance.frozen}₽')
        logger.info(f' · Active продажи: {Fore.LIGHTWHITE_EX}{profile.stats.deals.outgoing.total - profile.stats.deals.outgoing.finished}')
        logger.info(f' · Active покупки: {Fore.LIGHTWHITE_EX}{profile.stats.deals.incoming.total - profile.stats.deals.incoming.finished}')
        logger.info(f'{Fore.YELLOW}───────────────────────────────────────')
        ...
        if proxy:
            ...
            logger.info('')
            logger.info(f'{Fore.YELLOW}───────────────────────────────────────')
            logger.info(f'{Fore.YELLOW}Proxy information:')
