...
class PlayerokBot:
    ...
    def send_message(self, chat_id: str, text: str | None = None, photo_file_path: str | None = None,
        ...
        Tries send for 3 attempts, If Not succeeded - issues error V console.\n
        ...
        :param exclude_watermark: Skip And Not use water sign under message?
    ...
    def get_my_items(
        ...
        """
        Receives All items account.

        :param count: Number-in subjects, which need to get (Not more 24 for one request) or -1, If need to get All, _optional_.
        :type count: `int`
        ...
        :param statuses: Array statuses items, which need to get. Some statuses Can get only, If This profile your account. If Not indicated, receives straightaway All possible.
        :type statuses: `list[playerokapi.enums.ItemStatuses]` or `None`

        :return: Array items profile.
    ...
    def bump_item(self, item: ItemProfile | MyItem):
        try:
            ...
            logger.error(f"{Fore.LIGHTRED_EX}Error at raising subject «{item.name}»: {Fore.WHITE}{e}")
        ...
        try:
            ...
            if (
                ...
                if new_item.status in (ItemStatuses.PENDING_APPROVAL, ItemStatuses.APPROVED):
                    logger.info(f"{Fore.LIGHTWHITE_EX}«{item_name_frmtd}» {Fore.WHITE}— {Fore.YELLOW}product restored")
                else:
                    logger.error(f"{Fore.LIGHTRED_EX}Not succeeded restore item «{item_name_frmtd}». His status: {Fore.WHITE}{new_item.status.name}")
        except Exception as e:
            logger.error(f"{Fore.LIGHTRED_EX}Error at restoration subject «{item.name}»: {Fore.WHITE}{e}")
    ...
    def request_withdrawal(self) -> bool:
        ...
        try:
            ...
            logger.info(f"{Fore.LIGHTWHITE_EX}{balance}₽ {Fore.WHITE}— {Fore.YELLOW}transaction on conclusion created")
            return True
        except Exception as e:
        ...
        def refresh_account_loop():
            while True:
                ...
                try:
                    ...
                    logger.error(f"{Fore.LIGHTRED_EX}Error at update data about account: {Fore.WHITE}{traceback.format_exc()}")
        ...
        def check_banned_loop():
            while True:
                try:
                    ...
                    logger.error(f"{Fore.LIGHTRED_EX}Error at checks ban on account: {Fore.WHITE}{traceback.format_exc()}")
                time.sleep(900)
            ...
            while True:
                if self.config["playerok"]["auto_restore_items"]["expired"]:
                    try:
                        ...
                        logger.error(f"{Fore.LIGHTRED_EX}Error at auto-restoration expired items: {Fore.WHITE}{traceback.format_exc()}")
                time.sleep(45)
            ...
            while True:
                if (
                    ...
                    try:
                        ...
                        logger.error(f"{Fore.LIGHTRED_EX}Error at auto-raising items: {Fore.WHITE}{traceback.format_exc()}")
                time.sleep(3)
            ...
            while True:
                if (
                    ...
                    try:
                        ...
                        logger.error(f"{Fore.LIGHTRED_EX}Error at auto-conclusion funds: {Fore.WHITE}{traceback.format_exc()}")
                time.sleep(3)
    ...
    async def run_bot(self):
        ...
        if self.playerok_account.profile.balance:
            ...
            logger.info(f"   · IN waiting: {Fore.LIGHTWHITE_EX}{self.account.profile.balance.pending_income}₽")
