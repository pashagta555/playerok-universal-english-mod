...
class PlayerokBot:
    ...
    def check_banned(self):
        ...
        if user.is_blocked:
            ...
            self.logger.critical(f"{Fore.LIGHTRED_EX}Your Playerok account has been blocked! Unfortunately, I cannot continue working with a blocked account...")
    ...
    def send_message(self, chat_id: str, text: str | None = None, photo_file_path: str | None = None,
        ...
        """
        Custom method for sending a message to a Playerok chat.
        Tries up to 3 attempts; if it fails, logs an error to the console.\n
        ...
        :param exclude_watermark: Skip and do not append the watermark to the message.
        ...
        for _ in range(max_attempts):
            try:
                ...
                self.logger.error(f"{Fore.LIGHTRED_EX}Error while sending message {Fore.LIGHTWHITE_EX}«{msg}» {Fore.LIGHTRED_EX}to chat {Fore.LIGHTWHITE_EX}{chat_id} {Fore.LIGHTRED_EX}: {Fore.WHITE}{e}")
                return
        if text: msg = text.replace('\n', ' ').strip()
        else: msg = photo_file_path
        self.logger.error(f"{Fore.LIGHTRED_EX}Failed to send message {Fore.LIGHTWHITE_EX}«{msg}» {Fore.LIGHTRED_EX}to chat {Fore.LIGHTWHITE_EX}{chat_id}")
    ...
    def get_my_items(
        ...
        """
        Retrieves all items of the account.
        
        :param count: Number of items to retrieve (no more than 24 per request) or -1 to retrieve all, _optional_.
        :type count: `int`

        :param game_id: ID of the game/application whose items should be retrieved, _optional_.
        :type game_id: `str` or `None`

        :param category_id: ID of the game/application category whose items should be retrieved, _optional_.
        :type category_id: `str` or `None`

        :param statuses: Array of item statuses to retrieve. Some statuses are available only for your own account profile. If not specified, all possible statuses are retrieved.
        :type statuses: `list[playerokapi.enums.ItemStatuses]` or `None`

        :return: Array of item profiles.
    ...
    def bump_item(self, item: ItemProfile | MyItem):
        try:
            ...
            if (
                ...
                self.logger.info(f"{Fore.LIGHTWHITE_EX}«{item_name_frmtd}» {Fore.WHITE}— {Fore.YELLOW}bumped. {Fore.WHITE}Position: {Fore.LIGHTWHITE_EX}{sequence} {Fore.WHITE}→ {Fore.YELLOW}1")
        except Exception as e:
            self.logger.error(f"{Fore.LIGHTRED_EX}Error while bumping item «{item.name}»: {Fore.WHITE}{e}")
    ...
    def bump_items(self): 
        ...
        try:
            ...
            self.logger.error(f"{Fore.LIGHTRED_EX} Error while bumping items: {Fore.WHITE}{e}")
        ...
        try:
            ...
            if (
                ...
                if new_item.status in [ItemStatuses.PENDING_APPROVAL, ItemStatuses.APPROVED]:
                    ...
                    self.logger.error(f"{Fore.LIGHTRED_EX}Failed to restore item «{item_name_frmtd}». Its status: {Fore.WHITE}{new_item.status.name}")
        except Exception as e:
            self.logger.error(f"{Fore.LIGHTRED_EX}Error while restoring item «{item.name}»: {Fore.WHITE}{e}")
    ...
    def restore_expired_items(self):
        try:
            ...
            self.logger.error(f"{Fore.LIGHTRED_EX}Error while restoring expired items: {Fore.WHITE}{e}")
    ...
    def request_withdrawal(self) -> bool:
        ...
        try:
            ...
            if balance <= 500:
                raise Exception("Balance is too low. The transaction amount must be at least 500₽")
            ...
            self.logger.info(f"{Fore.LIGHTWHITE_EX}{balance}₽ {Fore.WHITE}— {Fore.YELLOW}withdrawal transaction created")
            return True
        except Exception as e:
            self.logger.error(f"{Fore.LIGHTRED_EX}Error while creating withdrawal transaction of {balance}₽: {Fore.WHITE}{e}")
        return False
    ...
    def log_new_review(self, deal: ItemDeal):
        ...
        self.logger.info(f" · Author: {Fore.LIGHTWHITE_EX}{deal.review.creator.username}")
    ...
    def log_deal_status_changed(self, deal: ItemDeal, status_frmtd: str = "Unknown"):
        ...
        self.logger.info(f"{Fore.WHITE}Deal status for {Fore.LIGHTWHITE_EX}{deal.id} {Fore.WHITE}has changed:")
    ...
    def log_new_problem(self, deal: ItemDeal):
        ...
        self.logger.info(f" · Author: {Fore.LIGHTWHITE_EX}{deal.user.username}")
        self.logger.info(f" · Item: {Fore.LIGHTWHITE_EX}{deal.item.name}")
        self.logger.info(f" · Amount: {Fore.LIGHTWHITE_EX}{deal.item.price}₽")
        self.logger.info(f"{Fore.YELLOW}───────────────────────────────────────")
    ...
    async def _on_new_message(self, event: NewMessageEvent):
        ...
        if (
            ...
            if str(event.message.text).lower() == "!commands":
                self.send_message(event.chat.id, self.msg("cmd_commands"))
            elif str(event.message.text).lower() == "!seller":
                asyncio.run_coroutine_threadsafe(
    ...
    async def _on_deal_status_changed(self, event: DealStatusChangedEvent):
        ...
        elif event.deal.status is ItemDealStatuses.PENDING: status_frmtd = "Waiting to be sent"
        ...
        elif event.deal.status is ItemDealStatuses.ROLLED_BACK: status_frmtd = "Refunded"
        ...
        if (
            ...
            asyncio.run_coroutine_threadsafe(
                get_telegram_bot().log_event(
                    log_text(
                        title=f'🔄️📋 Status of <a href="https://playerok.com/deal/{event.deal.id}/">deal</a> changed', 
    ...
    async def run_bot(self):
        ...
        if self.config["playerok"]["api"]["proxy"]:
            ...
            port = f"{port[:3]}**"
            user = f"{user[:3]}*****" if user else "No auth"
            password = f"{password[:3]}*****" if password else "No auth"
            self.logger.info(f"{ACCENT_COLOR}───────────────────────────────────────")
        ...
        self.logger.info(f"{Fore.YELLOW}Playerok bot started and active")
        # Bot started and active
