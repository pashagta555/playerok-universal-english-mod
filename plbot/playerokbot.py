...
class PlayerokBot:
    ...
    def bump_item(self, item: ItemProfile | MyItem):
        try:
            ...
            if (
                ...
                if not prem_status:
                    raise Exception("PREMIUM Status of the не найден")
        ...
        try:
            ...
            if (
                ...
                if new_item.status in (ItemStatuses.PENDING_APPROVAL, ItemStatuses.APPROVED):
                    ...
                    logger.error(f"{Fore.LIGHTRED_EX}Не удалось восстановить Item «{name_frmtd}». Его текущий Status of the: {Fore.WHITE}{new_item.status.name}")
        except Exception as e:
    ...
    def log_new_message(self, message: ChatMessage, chat: Chat):
        ...
        ch_header = f"New message in the chat с {chat_user}:"
    ...
    def log_new_deal(self, deal: ItemDeal):
        ...
        logger.info(f" · Price: {Fore.LIGHTWHITE_EX}{deal.item.price}₽")
        logger.info(f"{Fore.YELLOW}───────────────────────────────────────")
    ...
    def log_deal_status_changed(self, deal: ItemDeal, status_frmtd: str = "Неизвестный"):
        ...
        logger.info(f"{Fore.WHITE}Status of the deal {Fore.LIGHTWHITE_EX}{deal.id} {Fore.WHITE}was changed:")
        logger.info(f" · Status of the: {Fore.LIGHTWHITE_EX}{status_frmtd}")
        logger.info(f" · Buyer: {Fore.LIGHTWHITE_EX}{deal.user.username}")
        logger.info(f" · Товар: {Fore.LIGHTWHITE_EX}{deal.item.name}")
        logger.info(f" · Price: {Fore.LIGHTWHITE_EX}{deal.item.price}₽")
        logger.info(f"{Fore.WHITE}───────────────────────────────────────")
    ...
    def log_new_problem(self, deal: ItemDeal):
        logger.info(f"{Fore.YELLOW}───────────────────────────────────────")
        logger.info(f"{Fore.YELLOW}New жалоба в сделке {deal.id}:")
            ...
            if do:
                ...
                asyncio.run_coroutine_threadsafe(
                    get_telegram_bot().log_event(
                        text=log_text(
                            title=f'💬 New message in the <a href="https://playerok.com/chats/{event.chat.id}">chat</a>', 
                            text=text.strip()
                        ),
    ...
    async def _on_new_problem(self, event: ItemPaidEvent):
        ...
        if (
            ...
            asyncio.run_coroutine_threadsafe(
                get_telegram_bot().log_event(
                    text=log_text(
                        title=f'🤬 New жалоба в <a href="https://playerok.com/deal/{event.deal.id}">сделке</a>', 
                        text=(
    ...
    async def _on_deal_status_changed(self, event: DealStatusChangedEvent):
        ...
        if (
            ...
            asyncio.run_coroutine_threadsafe(
                get_telegram_bot().log_event(
                    log_text(
                        title=f'🔄️📋 Status of the <a href="https://playerok.com/deal/{event.deal.id}/">deal</a> was changed', 
                        text=f"<b>Новый Status of the:</b> {status_frmtd}"
                    )
                ), 
                get_telegram_bot_loop()
            )
