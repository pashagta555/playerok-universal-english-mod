...
class PlayerokBot:
    ...
    def check_banned(self):
        ...
        if user.is_blocked:
            ...
            self.logger.critical(f"{Fore.LIGHTRED_EX}Your Playerok account has been blocked! I cannot continue on a blocked account...")
            self.logger.critical(f"Contact Playerok support to find out the reason and resolve the issue.")
            self.logger.critical("")
            shutdown()
    ...
    def send_message(self, chat_id: str, text: str | None = None, photo_file_path: str | None = None,
        ...
        """
        Custom method to send a message to a Playerok chat.
        Tries up to 3 times; on failure logs an error to the console.
        Can send text `text` or a photo `photo_file_path`.
        ...
        :param photo_file_path: Path to photo file, optional.
        ...
        :param exclude_watermark: Skip watermark under the message?
        ...
        for _ in range(max_attempts):
            try:
                ...
                self.logger.error(f"{Fore.LIGHTRED_EX}Error sending message {Fore.LIGHTWHITE_EX}«{msg}» {Fore.LIGHTRED_EX}to chat {Fore.LIGHTWHITE_EX}{chat_id} {Fore.LIGHTRED_EX}: {Fore.WHITE}{e}")
                return
        if text: msg = text.replace('\n', ' ').strip()
        else: msg = photo_file_path
    ...
    def get_my_items(
        ...
        :param count: Number of items to fetch (max 24 per request) or -1 for all, optional.
        :param game_id: Game/app ID to filter by, optional.
        :param category_id: Category ID to filter by, optional.
        :param statuses: Item statuses to include. Some statuses only for own account. If None, returns all.
        :return: List of item profiles.
        """
    ...
    def bump_item(self, item: ItemProfile | MyItem):
        try:
            ...
            self.logger.error(f"{Fore.LIGHTRED_EX}Error bumping item «{item.name}»: {Fore.WHITE}{e}")
    ...
    def restore_item(self, item: Item | MyItem | ItemProfile):
        try:
            ...
            self.logger.error(f"{Fore.LIGHTRED_EX}Error restoring item «{item.name}»: {Fore.WHITE}{e}")
    ...
    def restore_expired_items(self):
        try:
            ...
            self.logger.error(f"{Fore.LIGHTRED_EX}Error restoring expired items: {Fore.WHITE}{e}")
    ...
    def request_withdrawal(self) -> bool:
        ...
        try:
            ...
            if balance <= 500:
                raise Exception("Balance too low. Transaction must be at least 500₽")
            ...
            self.logger.error(f"{Fore.LIGHTRED_EX}Error creating withdrawal transaction {balance}₽: {Fore.WHITE}{e}")
        return False
    ...
    def log_new_review(self, deal: ItemDeal):
        ...
        self.logger.info(f" · By: {Fore.LIGHTWHITE_EX}{deal.review.creator.username}")
    ...
    def log_deal_status_changed(self, deal: ItemDeal, status_frmtd: str = "Unknown"):
        ...
        self.logger.info(f"{Fore.WHITE}Deal status {Fore.LIGHTWHITE_EX}{deal.id} {Fore.WHITE}changed:")
    ...
    def log_new_problem(self, deal: ItemDeal):
        self.logger.info(f"{Fore.YELLOW}───────────────────────────────────────")
        self.logger.info(f"{Fore.YELLOW}New problem in deal {deal.id}:")
        self.logger.info(f" · By: {Fore.LIGHTWHITE_EX}{deal.user.username}")
        self.logger.info(f" · Item: {Fore.LIGHTWHITE_EX}{deal.item.name}")
        self.logger.info(f" · Amount: {Fore.LIGHTWHITE_EX}{deal.item.price}₽")
        self.logger.info(f"{Fore.YELLOW}───────────────────────────────────────")
    ...
    async def _on_new_problem(self, event: ItemPaidEvent):
        ...
        if (
            ...
            asyncio.run_coroutine_threadsafe(
                get_telegram_bot().log_event(
                    text=log_text(
                        title=f'🤬 New problem in <a href="https://playerok.com/deal/{event.deal.id}">deal</a>', 
    ...
    async def _on_deal_status_changed(self, event: DealStatusChangedEvent):
        ...
        elif event.deal.status is ItemDealStatuses.SENT: status_frmtd = "Seller confirmed"
        ...
        if (
            ...
            asyncio.run_coroutine_threadsafe(
                get_telegram_bot().log_event(
                    log_text(
                        title=f'🔄️📋 Deal <a href="https://playerok.com/deal/{event.deal.id}/">deal</a> status changed', 
    ...
    async def run_bot(self):
        self.logger.info("")
        self.logger.info(f"{ACCENT_COLOR}───────────────────────────────────────")
        self.logger.info(f"{ACCENT_COLOR}Account info:")
        ...
        if self.config["playerok"]["api"]["proxy"]:
            ...
            self.logger.info(f"{ACCENT_COLOR}───────────────────────────────────────")
            self.logger.info(f"{ACCENT_COLOR}Proxy info:")
        ...
        self.logger.info(f"{Fore.YELLOW}Playerok bot is running")
