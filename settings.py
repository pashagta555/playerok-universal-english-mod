    ...
    default={
        "first_message": {
            ...
            "text": [
                ...
                "",
                "💡 If you want to talk to the seller, send the command `!seller` and I will invite them to this chat.",
                "",
                "To see all available commands, send `!commands`."
            ]
        },
        "cmd_error": {
            "enabled": True,
        ...
        "cmd_seller": {
            "enabled": True,
            "text": [
                "💬 The seller has been called to this chat. Please wait for them to join the conversation..."
            ]
        },
        "new_deal": {
            "enabled": False,
            "text": [
                ...
                "",
                "The seller may be away right now. To call them, use the `!seller` command."
            ]
        },
        "deal_pending": {
            "enabled": False,
            "text": [
                "⌛ Please send the required details so I can complete your order."
            ]
        },
        "deal_sent": {
            "enabled": False,
            "text": [
                "✅ I marked your order as completed! If you did not receive the purchased item, please write in this chat."
            ]
        },
        "deal_confirmed": {
            "enabled": False,
            "text": [
                "🌟 Thank you for a successful deal. I would appreciate your review. See you again in my store, and good luck!"
            ]
        },
        "deal_refunded": {
            "enabled": False,
            "text": [
                "📦 The order was refunded. I hope this did not cause inconvenience. See you again in my store, and good luck!"
            ]
        },
        "new_review": {
            "enabled": False,
            "text": [
                "✨ Thank you for your {review_rating}⭐ review! I hope you were satisfied with the quality of service."
            ]
        }
    }
)
...
def validate_config(config, default):
    """
    Validate config structure against the default template.
...
def restore_config(config: dict, default: dict):
    """
    Restore missing parameters from the default template.
    Also removes invalid values by replacing them with defaults.
...
def get_json(path: str, default: dict, need_restore: bool = True) -> dict:
    ...
    Restores missing fields when needed.
    ...
    :param need_restore: Whether to validate and restore config fields.
...
def set_json(path: str, new: dict):
    """
    Write new data to the settings file.
